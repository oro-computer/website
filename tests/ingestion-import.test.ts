...
async function page(site: string, route: string, collection: string, sourcePath: string, body: string): Promise<string> {
  ...
  await put(path, pageText({ layout: collection === 'silk' && sourcePath === 'spec/2026.md' ? 'spec' : 'docs', title: 'Authored title', description: 'Authored description', docsCollection: collection, sourcePath, order: 7, section: 'authored', githubRepo: 'preserved/repository', githubRef: 'authored-ref' }, body))
  return path
}
test('Runtime generator uses the public importer, prunes excluded families, and retains curated metadata', async () => temporary(async (site, upstream) => {
  ...
  assert.equal(metadata.layout, 'docs')
  ...
  assert.equal(splitPage(first).metadata.layout, 'docs')
  await generateReference(site, upstream)
...
test('Silk imports prune upstream-owned pages but retain authored pages and an absent upstream wiki', async () => temporary(async (site, upstream) => {
  ...
  await put(join(upstream, 'docs/spec/2026.md'), '# Silk specification\n')
  ...
  assert.equal(splitPage(first).metadata.layout, 'docs')
  assert.equal(splitPage(await readFile(join(site, 'src/silk/spec/2026/page.md'), 'utf8')).metadata.layout, 'spec')
  ...
  const fresh = splitPage(await readFile(join(site, 'src/silk/wiki/fresh/page.md'), 'utf8'))
    assert.equal(fresh.metadata.layout, 'docs')
    assert.ok(fresh.body.includes('# Fresh wiki'))
}))
