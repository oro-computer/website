...
Our embedding ABI for `libsilk.a` will treat `SilkString` as the canonical C representation; details are further specified in [abi libsilk](/silk/docs/compiler/abi-libsilk/).
...
- `i128` → `SilkI128` (see [abi libsilk](/silk/docs/compiler/abi-libsilk/); `{ lo, hi }` lanes)
- `u128` → `SilkU128` (see [abi libsilk](/silk/docs/compiler/abi-libsilk/); `{ lo, hi }` lanes)
- `int` → `int64_t` (current `linux/x86_64` baseline; do not assume C `int`)
- `f32` → `float`
- `f64` → `double`
- `f128` → `SilkF128` (see [abi libsilk](/silk/docs/compiler/abi-libsilk/); IEEE binary128 bits in `{ lo, hi }`)
- `bool` → `bool` (or `_Bool`)
...
- describe how user‑defined structs map to C structs (respecting the layout rules in [`structs-impls-layout.md`](/silk/docs/language/structs-impls-layout/)),
...
- Keep this document and [abi libsilk](/silk/docs/compiler/abi-libsilk/) in sync with the actual codegen strategy.
