...
section: "language"
order: 12
sourcePath: "language/cheat-sheet.md"
...
- [implementation status](/silk/docs/compiler/implementation-status/), and
...
- Runtime `let`/`var` bindings and compile-time `const` bindings must have an initializer ([diagnostics](/silk/docs/compiler/diagnostics/), `E2015`).
...
- `const` initializers must be compile-time evaluable ([diagnostics](/silk/docs/compiler/diagnostics/), `E2041`); in the Supported forms this is restricted to scalar expressions and calls to `const fn` functions (still no `/` or `%`), plus string literals / `const` string aliases.
...
- Function expressions (non-capturing, inferred `pure`):
 ...
 - capturing closures are supported as a subset; see [types](/silk/docs/language/types/).
...
Operator precedence and associativity follow the rules in [operators](/silk/docs/language/operators/).
...
- `gpu (grid=<u64>, workspace=<u64>) { kernel(args...); }` (checked,
 ...
 [`std::gpu::DispatchResult`](/silk/docs/std/gpu/) in value position and is discardable in statement
 position; see [`gpu-launch-blocks.md`](/silk/docs/language/gpu-launch-blocks/)).
- Blocks: `{ stmt* }`.
...
See [`structs-impls-layout.md`](/silk/docs/language/structs-impls-layout/) and [`interfaces.md`](/silk/docs/language/interfaces/) for details.
...
- Allocation:
 - `new` uses the active region inside `with` (see [`regions.md`](/silk/docs/language/regions/)).
...
See [`concurrency.md`](/silk/docs/language/concurrency/) for deeper semantics.
...
`#require` / `#assure` appear before functions; `#invariant` / `#variant` / `#monovariant` appear before loops; `#const` and `#assert` appear inside blocks. See [`formal-verification.md`](/silk/docs/language/formal-verification/).
...
See [`ext.md`](/silk/docs/language/ext/) and [abi libsilk](/silk/docs/compiler/abi-libsilk/) for full details.
