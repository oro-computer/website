...
- `const pure fn` is simply a `const fn` that also opts into the `pure` rules
 (see [`function-disciplines.md`](/silk/docs/language/function-disciplines/)).
...
introduced by the literal syntax itself (see [`literals-string.md`](/silk/docs/language/literals-string/)), not by the
`const fn` evaluator.
