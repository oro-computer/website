...
- The type system ([`src/types.zig`](https://github.com/oro-computer/silk/blob/master/src/types.zig)) models optional types, and the parser
 now accepts both:
 ...
 [diagnostics](/silk/docs/compiler/diagnostics/), `E2015`):
...
- The current `linux/x86_64` IR→ELF backend subset now supports a first slice
 ...
 including nested structs and optionals; see [structs impls layout](/silk/docs/language/structs-impls-layout/)).
...
within the supported backend subset, see [abi libsilk](/silk/docs/compiler/abi-libsilk/) and
