...
section: "language"
order: 11
sourcePath: "language/syntax-tour.md"
...
- [grammar](/silk/docs/language/grammar/) (the exact grammar the parser accepts),
- the concept documents under `docs/language/` (semantics and checker rules),
- [implementation status](/silk/docs/compiler/implementation-status/) (what works end-to-end today),
- and [diagnostics](/silk/docs/compiler/diagnostics/) (error codes for unsupported forms).
...
- [grammar](/silk/docs/language/grammar/) for syntax,
- [implementation status](/silk/docs/compiler/implementation-status/) for current end-to-end support,
- [`tests/silk/pass_*.slk`](https://github.com/oro-computer/silk/blob/master/tests/silk/pass_*.slk) for working example programs.
...
- The entrypoint for an executable build is `main` returning `int` (see
 [cli silk](/silk/docs/compiler/cli-silk/) for the CLI rules and supported targets).
...
position it returns [`std::gpu::DispatchResult`](/silk/docs/std/gpu/), which exposes separate launch
...
[gpu launch blocks](/silk/docs/language/gpu-launch-blocks/); use separate manual [`std::gpu`](/silk/docs/std/gpu/) calls when
...
Doc comments (tooling-only; see [doc comments](/silk/docs/language/doc-comments/)):
...
[formal verification](/silk/docs/language/formal-verification/)).
...
Top-level ordering is enforced (see [packages imports exports](/silk/docs/language/packages-imports-exports/)):
...
See [packages imports exports](/silk/docs/language/packages-imports-exports/) for the full import/export model.
...
- Only `let mut`/`var` bindings are assignable lvalues (see
 [mutability](/silk/docs/language/mutability/) and [operators](/silk/docs/language/operators/)).
...
`pure fn` (restricted subset; see [function disciplines](/silk/docs/language/function-disciplines/)):
...
Varargs (final parameter prefixed by `...`; see [varargs](/silk/docs/language/varargs/)):
...
Capturing closures are supported as a restricted subset; see [types](/silk/docs/language/types/)
and [memory model](/silk/docs/language/memory-model/).
...
Optional kind tags (validated by the checker; see [types](/silk/docs/language/types/)):
...
Attach methods with `impl` (see [structs impls layout](/silk/docs/language/structs-impls-layout/)):
...
Single inheritance (current surface is implemented; see [structs impls layout](/silk/docs/language/structs-impls-layout/)):
...
See [enums](/silk/docs/language/enums/) and [flow match](/silk/docs/language/flow-match/).
...
See [interfaces](/silk/docs/language/interfaces/).
...
See [typed errors](/silk/docs/language/typed-errors/).

### 4.9 External declarations: `ext`

External function binding (symbol name optional; see [ext](/silk/docs/language/ext/)):
...
[varargs](/silk/docs/language/varargs/) and [ext](/silk/docs/language/ext/).

See also: [abi libsilk](/silk/docs/compiler/abi-libsilk/) (C ABI) and [`include/silk/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk/silk.h).
...
See [testing](/silk/docs/language/testing/) and run with `silk test`.
...
See [types](/silk/docs/language/types/) for full details and implementation limits.
...
See [generics](/silk/docs/language/generics/) for Supported forms limits (notably `E2016` for
const parameters/arguments and generic functions).

## 6. Statements (Inside Blocks)

The statement grammar is summarized in [grammar](/silk/docs/language/grammar/) and detailed
...
`for` over a range form (special-cased surface; see [flow for](/silk/docs/language/flow-for/)):
...
`panic` is used for typed errors (see [typed errors](/silk/docs/language/typed-errors/)):
...
See [typed errors](/silk/docs/language/typed-errors/) for the Terminal Arm Rule and the supported
pattern forms.
...
productions, see [grammar](/silk/docs/language/grammar/).
...
for scalar types (see [operators](/silk/docs/language/operators/)).
...
See [formal verification](/silk/docs/language/formal-verification/) for the exact verifier model and
...
- Syntax: [grammar](/silk/docs/language/grammar/)
- Types: [types](/silk/docs/language/types/), [generics](/silk/docs/language/generics/)
- Operators: [operators](/silk/docs/language/operators/)
- Flow control: [flow overview](/silk/docs/language/flow-overview/) and `docs/language/flow-*.md`
- Modules/imports/exports: [packages imports exports](/silk/docs/language/packages-imports-exports/)
...
- Formal verification: [formal verification](/silk/docs/language/formal-verification/)
