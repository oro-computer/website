...
section: "language"
order: 56
sourcePath: "language/gpu-launch-blocks.md"
...
[`gpu-execution.md`](/silk/docs/language/gpu-execution/): it is a normal, non-generic root-package
...
[`std::gpu::launch_and_synchronize`](/silk/docs/std/gpu/) implementation, so the source file need not
...
[`std::gpu::DispatchResult`](/silk/docs/std/gpu/) with independent `launch_status` and
...
See [`gpu-execution.md`](/silk/docs/language/gpu-execution/) for placement and device-call rules,
[gpu](/silk/docs/std/gpu/) for buffers and synchronization, and
[backend gpu](/silk/docs/compiler/backend-gpu/) for the shared backend boundary.
