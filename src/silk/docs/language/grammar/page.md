...
- Whitespace and comments (trivia):
 ...
 documentation generation as specified in [doc comments](/silk/docs/language/doc-comments/).
- Identifiers:
 ...
 file imports: `from`,
 - control flow and host dispatch: `if`, `else`, [`loop`](/silk/wiki/language/flow-loop/), [`while`](/silk/wiki/language/flow-while/), `for`, `in`, [`match`](/silk/wiki/language/flow-match/), `return`, `panic`, `break`, `continue`, `assert`, `await`, `yield`, `gpu`,
...
- Literals:
 ...
 - `/pattern/flags` (see [literals regexp](/silk/docs/language/literals-regexp/)).
...
- Operators and delimiters:
 - as listed in [operators](/silk/docs/language/operators/) (assignment, arithmetic, logical, bitwise, ranges, `?.`, `??`, `::`, `->`, `=>`, etc.).
...
- Packages and imports:
 ...
 [packages imports exports](/silk/docs/language/packages-imports-exports/). The grammar in this file mirrors
 ...
 **final**,
 ...
 the checker (see [diagnostics](/silk/docs/compiler/diagnostics/), `E2015`).

 Additionally, `const` initializers must be compile-time evaluable; otherwise
 the compiler reports an error (see [diagnostics](/silk/docs/compiler/diagnostics/), `E2041`).
...
- Types (Supported forms):
 ...
 closures as a restricted scalar-only subset; see [types](/silk/docs/language/types/) and
...
- Statements (Supported forms):
 ...
 through [`std::gpu::launch_and_synchronize`](/silk/docs/std/gpu/), which invokes synchronization
 exactly once after launch. It has type [`std::gpu::DispatchResult`](/silk/docs/std/gpu/); statement
 position may omit the trailing semicolon and discards that value. See
 [gpu launch blocks](/silk/docs/language/gpu-launch-blocks/).
 ...
 specified in [typed errors](/silk/docs/language/typed-errors/).
...
- Expressions (Supported forms):
 ...
 [`src/parser.zig`](https://github.com/oro-computer/silk/blob/master/src/parser.zig):
...
- Declarations (Supported forms additions):
 ...
 see [cli silk](/silk/docs/compiler/cli-silk/) for the exact supported subset.

Further expression forms (ranges, etc.) are described in other
language concept documents and in [operators](/silk/docs/language/operators/). The current
