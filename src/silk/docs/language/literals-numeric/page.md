...
- Duration literal tokens of the form `<number><unit>` (no whitespace) such as
 `500ms` and `1.5s` (specified in [literals duration](/silk/docs/language/literals-duration/)).
...
See [types](/silk/docs/language/types/) for the primitive type names used below.
...
([duration instant](/silk/docs/language/duration-instant/)), so integer literals can be used as
...
- **Mixing integers and floats implicitly**: use `as` casts ([operators](/silk/docs/language/operators/))
 to convert explicitly when you need to combine integer and float values.

## Related Documents

- [literals duration](/silk/docs/language/literals-duration/) (duration literals like `5ms`, `1.5s`)
- [duration instant](/silk/docs/language/duration-instant/) (time types and operators)
- [operators](/silk/docs/language/operators/) (unary `-`, arithmetic, and `as` casts)
- [types](/silk/docs/language/types/) (primitive numeric type names)
