...
[const functions](/silk/docs/language/const-functions/). The `const` modifier is orthogonal to the
...
- Lowering/codegen implements `task` execution using OS threads on `linux/x86_64`
 ...
 ([`src/silk_rt_async.c`](https://github.com/oro-computer/silk/blob/master/src/silk_rt_async.c)) so `await` is a true suspension point:
 ...
 [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) API; see [async runtime](/silk/docs/compiler/async-runtime/).
