...
section: "compiler"
order: 265
sourcePath: "compiler/backend-amdgpu.md"
...
lexical function name through the manual [`std::gpu::launch`](/silk/docs/std/gpu/) API. The checked
form uses [`std::gpu::launch_and_synchronize`](/silk/docs/std/gpu/) and returns both phase statuses;
...
[backend gpu](/silk/docs/compiler/backend-gpu/) and also carries CUDA/PTX bundles.
...
device IR specified in [`backend-gpu.md`](/silk/docs/compiler/backend-gpu/). Pure-Silk applications import semantic
operations from [`std::gpu::device`](/silk/docs/std/gpu-device/). The lower-level [`std::gpu::isa`](/silk/docs/std/gpu-isa/) surface
...
[`std::gpu`](/silk/docs/std/gpu/) owns runtime discovery, buffers, launch, and synchronization. For an
...
- declarations owned by [`std::gpu::isa`](/silk/docs/std/gpu-isa/) are compiler-classified device
...
target-neutral GPU-v1 lowering described in [`backend-gpu.md`](/silk/docs/compiler/backend-gpu/) instead.
...
- `AMDGPU source kernel parameters must be immutable u64 values`: change the
 ...
 trailing `u64` value per parameter to [`std::gpu::launch`](/silk/docs/std/gpu/).
...
- In the standalone `--kind object --target amdgcn-...` source-intrinsic path,
 ...
 [`std::gpu::isa`](/silk/docs/std/gpu-isa/), [`std::gpu::device::store_u32_at_global_x`](/silk/docs/std/gpu-device/),
 [`std::gpu::device::classify_u32_at_global_x`](/silk/docs/std/gpu-device/), and `return;` for now; `let`,
 `if`, [`while`](/silk/wiki/language/flow-while/), arbitrary calls, and assignments are not part of this path.
 Mixed host/device applications instead lower the portable subset and eligible
 helper call graph through [`src/lower_gpu_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_gpu_ir.zig).
...
- compiler-generated explicit-kernarg loads and global-X address calculation
 for [`std::gpu::device::store_u32_at_global_x`](/silk/docs/std/gpu-device/), including the processor-aware
...
- compiler-generated packed input load, unsigned threshold comparison, and
 ...
 [`std::gpu::device::classify_u32_at_global_x`](/silk/docs/std/gpu-device/) operation,
