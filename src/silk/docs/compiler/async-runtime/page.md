...
section: "compiler"
order: 268
sourcePath: "compiler/async-runtime.md"
...
- The shipped hosted async runtime implementation in [`src/silk_rt_async.c`](https://github.com/oro-computer/silk/blob/master/src/silk_rt_async.c) and
 [`src/silk_rt_task_pool.c`](https://github.com/oro-computer/silk/blob/master/src/silk_rt_task_pool.c) is supported on the current hosted POSIX compiler
...
- Specify the longer-term architecture (compiler coroutine transform + richer event loop)
 that the implementation is expected to evolve toward.

The current hosted async runtime is implemented in C in [`src/silk_rt_async.c`](https://github.com/oro-computer/silk/blob/master/src/silk_rt_async.c) and wired
into lowering in [`src/lower_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_ir.zig). It provides:
...
- language model and Supported forms: [concurrency](/silk/docs/language/concurrency/)
- function disciplines: [function disciplines](/silk/docs/language/function-disciplines/)
- runtime layering: [runtime](/silk/docs/std/runtime/)
- current blocking hosted utilities: [task](/silk/docs/std/task/), [io](/silk/docs/std/io/), [networking](/silk/docs/std/networking/)
...
[`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/)):
...
These are intended to be wrapped by stable [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) / [`std::task`](/silk/docs/std/task/) /
[`std::io`](/silk/docs/std/io/) surfaces as that layer is brought up. In the current stdlib snapshot,
timers and fd readiness are exposed via [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/), and sleep
helpers are wrapped as [`std::task::{sleep_ms_async,sleep_async}`](/silk/docs/std/task/). As of the
...
- The runtime syncs coroutine and executor contexts to the current thread mask before
 ...
 This is required for thread-level signal masking patterns like [`std::signal`](/silk/docs/std/signal/)
 (`signalfd(2)` on Linux).
...
- The [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) handle surface is still limited:
 - only one global executor/event loop instance may be active at a time,
 - [`std::runtime::event_loop::init()`](/silk/docs/std/runtime-event_loop/) fails when an executor is already active
...
- Keep the runtime **pluggable** via the [`std::runtime::...`](/silk/docs/std/runtime/) layering:
 - higher-level `std::...` modules should rely on stable [`std::runtime::...`](/silk/docs/std/runtime/) interfaces,
...
([cli silk](/silk/docs/compiler/cli-silk/)). With a real executor, the entry stub for executables will:
...
Structured concurrency forms (`async { ... }`, `task { ... }`, and their [`loop`](/silk/wiki/language/flow-loop/)
...
- no automatic [`std::abort_controller`](/silk/docs/std/abort_controller/) injection,
...
## Runtime Layering ([`std::runtime`](/silk/docs/std/runtime/))

The async executor and event loop will be exposed under [`std::runtime`](/silk/docs/std/runtime/) as a new runtime area.

### Proposed new runtime area

- [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) — stable interface used by the compiler-generated coroutine
 runtime and by async-aware stdlib code.
- Backends:
 - [`std::runtime::linux::event_loop`](/silk/docs/std/runtime-linux-event_loop/) — Linux `io_uring` implementation (preferred).
 - [`std::runtime::posix::event_loop`](/silk/docs/std/runtime-posix-event_loop/) — portable readiness-based fallback (poll/epoll).
 - [`std::runtime::wasi::event_loop`](/silk/docs/std/runtime-wasi-event_loop/) — WASI implementation (likely limited; may be timers +
...
The exact Silk-level signatures are specified in [`std/runtime/event_loop.slk`](https://github.com/oro-computer/silk/blob/master/std/runtime/event_loop.slk) and are expected
...
2. **Portable I/O readiness backend**:
 - expose async fd readiness in [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/),
 - add async wrappers in [`std::io`](/silk/docs/std/io/) / [`std::net`](/silk/docs/std/networking/) (opt-in, minimal initial surface).
