...
section: "compiler"
order: 267
sourcePath: "compiler/backend-wasm.md"
...
- Supported targets:
 ...
 [`std::args::{argc,argv,current}`](/silk/docs/std/args/), backed by WASI `args_sizes_get` / `args_get`.
...
`silk build --arch wasm32|wasm32-wasi` (see [CLI reference](/silk/docs/compiler/cli-silk/)
and [`silk(1)`](/silk/docs/man/silk.1/)).
...
- `wasm32-wasi`:
 ...
 `proc_exit(exit_code)`,
 - argv access stays inside `main()` through [`std::args`](/silk/docs/std/args/); `_start` does not
...
[External declarations (`ext`)](/silk/docs/language/ext/) (WASM-specific subsection) and in
[CLI reference](/silk/docs/compiler/cli-silk/).

### WASI integration

For `wasm32-wasi`, shipped facilities such as [`std::io`](/silk/docs/std/io/), [`std::args`](/silk/docs/std/args/),
[`std::env`](/silk/docs/std/env/), and supported [`std::fs`](/silk/docs/std/fs/) operations dispatch through the
target-specific [`std::runtime::wasi::*`](/silk/docs/std/runtime/) modules rather than the POSIX
