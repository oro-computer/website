...
section: "compiler"
order: 269
sourcePath: "compiler/ir-overview.md"
...
[`src/lower_gpu_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_gpu_ir.zig) produces an `IrProgram` for each launchable device entry
...
- the lowering passes in [`src/lower_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_ir.zig) to ensure that arithmetic,
 comparisons, and boolean expressions are mapped to the correct instruction
 shapes,
- the IR interpreter in [`src/ir_eval.zig`](https://github.com/oro-computer/silk/blob/master/src/ir_eval.zig) to maintain a consistent runtime
 encoding (everything is stored in 8-byte slots, with `Bool` encoded as
 `0`/`1` and floats stored as raw IEEE bits), and
- the ELF backend in [`src/backend_ir_elf.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_ir_elf.zig) to choose the appropriate
...
## Multi-Scalar Values at ABI Boundaries
...
- `string` is lowered as `{ ptr: u64, len: i64 }` and returns via `rax`/`rdx` on
 `linux/x86_64` (see [ext](/silk/docs/language/ext/) and [abi libsilk](/silk/docs/compiler/abi-libsilk/)).
...
- For the initial `struct` subset (see [structs impls layout](/silk/docs/language/structs-impls-layout/)),
...
- `Load`
 ...
 the IR interpreter currently treats it as unsupported.
- `Store`
 - store an 8-byte scalar value from [`src`](https://github.com/oro-computer/silk/tree/master/src) to memory at address `ptr`.
 - `ptr` must have scalar type `U64`.
...
- `Call`
 ...
 - in the Supported forms, calls are:
...
- The IR is now part of the primary executable build path on `linux/x86_64`:
 ...
 - [`src/lower_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_ir.zig) lowers supported programs into `ir.Function` /
 `ir.Program` graphs,
 - [`src/backend_ir_elf.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_ir_elf.zig) lowers those IR graphs directly to a minimal
...
- A constant-expression fallback remains in place for programs that type-check
 ...
 - [`src/backend_ir_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_ir_const.zig) attempts IR lowering + interpretation for
 constant-style `fn main() -> int` programs using [`src/ir_eval.zig`](https://github.com/oro-computer/silk/blob/master/src/ir_eval.zig),
 - if that fails, callers fall back to the legacy AST-based constant
 evaluator in [`src/backend_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_const.zig) (this also supports the standard
 `fn main(argc: int, argv: u64) -> int` form when the return value is
 constant and does not depend on arguments),
 - [`src/backend_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_const.zig) emits a minimal target-specific const-main stub
...
package graphs are described in [architecture](/silk/docs/compiler/architecture/); the IR is
one of the layers that will carry that information down to the back-end.

## Lowering Structured Control Flow

Structured control flow in the surface language (`if` / `else` and [`while`](/silk/wiki/language/flow-while/))
...
- `if` / `else` statements become:
 ...
 or continues).
- [`while`](/silk/wiki/language/flow-while/) loops become:
...
- A narrow, constant‑oriented lowering in [`src/lower_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_ir.zig) is wired into
 the compiler:
 ...
 and a restricted control‑flow surface (constant [`while`](/silk/wiki/language/flow-while/) and terminating
...
- More general lowering of `if`/`else` and [`while`](/silk/wiki/language/flow-while/) throughout function
...
- [`src/ir.zig`](https://github.com/oro-computer/silk/blob/master/src/ir.zig) defines the initial IR data structures and minimal operations
 for:
 ...
 entry index.
- [`src/ir_eval.zig`](https://github.com/oro-computer/silk/blob/master/src/ir_eval.zig) provides IR execution helpers that:
...
- Tests in [`src/tests.zig`](https://github.com/oro-computer/silk/blob/master/src/tests.zig) construct:
...
- [`src/lower_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/lower_ir.zig) implements lowering passes from checked AST to IR:
 ...
 - [`while`](/silk/wiki/language/flow-while/) loops whose conditions are compile-time boolean literals
 ...
 - or, in the growing loop subset, contains one or more [`while`](/silk/wiki/language/flow-while/) loops
 ...
 or `continue;` statements (no nested `if` / [`while`](/silk/wiki/language/flow-while/) or `return` in
 ...
 statements, [`while`](/silk/wiki/language/flow-while/) loops with boolean conditions, `break` /
...
- [`src/backend_ir_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_ir_const.zig) integrates this lowering and the IR interpreter
 into the executable build paths:
 - the CLI ([`src/driver.zig`](https://github.com/oro-computer/silk/blob/master/src/driver.zig)) and the C ABI ([`src/abi.zig`](https://github.com/oro-computer/silk/blob/master/src/abi.zig)) first attempt
 ...
 in [`src/backend_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_const.zig),
 - in both cases, [`src/backend_const.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_const.zig) is still responsible for emitting
 the minimal target-specific executable used by the constant-only backend.
- [`src/backend_ir_elf.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_ir_elf.zig) provides an IR→ELF64 backend for `linux/x86_64`
 ...
 `lowerIntFunctionCfg` so that nested `if` / `else` and [`while`](/silk/wiki/language/flow-while/)
...
- document and implement IR support for the language’s richer expression
 ...
 combinations, and eventually higher-level constructs such as [`match`](/silk/wiki/language/flow-match/)),
