...
section: "compiler"
order: 274
sourcePath: "compiler/abi-libsilk.md"
...
- Mirror the external-declaration semantics described in [ext](/silk/docs/language/ext/).
...
- Configuration:
 ...
 backend ([`src/backend_wasm_ir.zig`](https://github.com/oro-computer/silk/blob/master/src/backend_wasm_ir.zig)), with a smaller constant-only fallback for
 ...
 [`std::gpu::device::store_u32_at_global_x`](/silk/docs/std/gpu-device/) and
 [`std::gpu::device::classify_u32_at_global_x`](/silk/docs/std/gpu-device/) helpers accept direct kernel
 parameter names.
 ...
 [backend amdgpu](/silk/docs/compiler/backend-amdgpu/) documents the canonical source-intrinsic
...
- Building artifacts:
 ...
 [backend amdgpu](/silk/docs/compiler/backend-amdgpu/): header at `0..2`, setup at `2..4`,
 ...
 [backend amdgpu](/silk/docs/compiler/backend-amdgpu/). Arbitrary Silk IR lowering is not
 ...
 in both Zig tests and C tests (see [`c-tests/build_exec_helper_params*.c`](https://github.com/oro-computer/silk/blob/master/c-tests/build_exec_helper_params*.c)), or
...
- Error reporting:
 ...
 - The text format and initial stable error code set are specified in [diagnostics](/silk/docs/compiler/diagnostics/). Embedders should treat the formatted message as user-facing text (not a stable machine-readable protocol).
...
Any deviation from the mappings documented in [ext](/silk/docs/language/ext/) must be justified here and reflected in tests.
