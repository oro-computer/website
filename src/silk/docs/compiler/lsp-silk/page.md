...
section: "compiler"
order: 273
sourcePath: "compiler/lsp-silk.md"
...
No language features or CLI options are introduced by the LSP itself. Any future extensions that affect language semantics or user-facing flags must still be documented in the appropriate `docs/language/` or [cli silk](/silk/docs/compiler/cli-silk/) files first.
...
- Local scopes are then consulted to resolve:
 ...
 - [`match`](/silk/wiki/language/flow-match/)-statement binders within the selected arm body.
...
- Completion items are offered for:
 - all language keywords defined in [`src/token.zig`](https://github.com/oro-computer/silk/blob/master/src/token.zig) (via `keywordTable()`),
 ...
 / `while let` and chained `&& let` clauses, [`match`](/silk/wiki/language/flow-match/) statement arms, and
...
- Document symbols are produced hierarchically from the parsed AST:
 ...
 - functions, tests, loops, [`match`](/silk/wiki/language/flow-match/) arms, and similar nested bodies surface supported local declarations and binders beneath their containing symbol.
...
- walk upward from the `silk-lsp` executable’s directory to find [`std/`](https://github.com/oro-computer/silk/tree/master/std/) (developer build fallback),
- if none of the above are available and the LSP client provides `rootUri`/`rootPath`, walk upward from that workspace root to find a [`std/`](https://github.com/oro-computer/silk/tree/master/std/) directory.
...
- Parsing uses `parser.Parser` and the existing grammar in [grammar](/silk/docs/language/grammar/).
- Type checking uses `checker.checkModule` and the rules from [types](/silk/docs/language/types/) and related concept docs.
...
- [architecture](/silk/docs/compiler/architecture/) (compiler and tool layout),
- [cli silk](/silk/docs/compiler/cli-silk/) (CLI behavior for `silk`),
