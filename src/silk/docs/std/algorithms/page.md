...
This describes intended algorithm APIs; it is not
implemented yet.

[`std::algorithms`](/silk/docs/std/algorithms/) provides common algorithms over collections (primarily
[`std::arrays::Slice(T)`](/silk/docs/std/arrays/) and related types).

See also:

- [arrays](/silk/docs/std/arrays/)
- [conventions](/silk/docs/std/conventions/)

## Notes

A tiny, non-generic subset is implemented in [`std/algorithms.slk`](https://github.com/oro-computer/silk/blob/master/std/algorithms.slk) for early
...
[`std::algorithms`](/silk/docs/std/algorithms/) is responsible for:

- Searching, sorting, and transformation routines.
- Designed to work with [`std::arrays`](/silk/docs/std/arrays/) and other iterable types.
