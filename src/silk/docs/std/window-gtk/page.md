...
[`std::window::gtk`](/silk/docs/std/window-gtk/) is the GTK-specific facade for applications that want to
...
bundled Linux runtime loads GTK dynamically when [`std::window`](/silk/docs/std/window/) code is reached.
...
- `is_supported()` returns `true` only when the runtime selected
 [`std::window::Backend::GTK`](/silk/docs/std/window/).
- `open(...)` delegates to [`std::window::open(...)`](/silk/docs/std/window/) after checking that the GTK
 provider is active.
...
- The portable [`std::window::next_event(...)`](/silk/docs/std/window/), `poll(...)`, `close(...)`, and
