...
[`std::url`](/silk/docs/std/url/) provides a WHATWG-URL-compatible URL parser and serializer plus `URLSearchParams` behavior (`application/x-www-form-urlencoded`).
...
`URLResult` is [`std::result::Result(URL, ParseError)`](/silk/docs/std/result/):

- `Ok(URL)` on success.
- `Err(ParseError)` on error.

Since `URL` owns heap allocations and implements [`std::interfaces::Drop`](/silk/docs/std/interfaces/), prefer
...
`URL` also implements [`std::interfaces::Parse(ParseError)`](/silk/docs/std/interfaces/), so the
receiverless convenience method `URL.parse(input: string) -> URLResult`
forwards to [`std::url::parse`](/silk/docs/std/url/) through the standard static protocol surface.

### URL record

`URL` is an owned URL record with accessor methods:

- `URL` owns heap allocations and implements [`std::interfaces::Drop`](/silk/docs/std/interfaces/); it is
...
- `URL` also implements [`std::interfaces::TrySerialize(std::memory::OutOfMemory)`](/silk/docs/std/interfaces/);
...
- `URLSearchParams` owns its query buffer and implements [`std::interfaces::Drop`](/silk/docs/std/interfaces/),
 [`std::interfaces::Len`](/silk/docs/std/interfaces/), [`std::interfaces::IsEmpty`](/silk/docs/std/interfaces/),
 [`std::interfaces::Serialize(string)`](/silk/docs/std/interfaces/), and
...
- `URLSearchParams` uses the same zero-capacity-empty / trailing-NUL storage
 invariant as [`std::strings::String`](/silk/docs/std/strings/), so `let q: string = params as string;`
