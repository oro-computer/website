...
[`std::flag`](/silk/docs/std/flag/) provides a small, robust command
...
[`std::flag`](/silk/docs/std/flag/) is intended for programs that can build a [`std::args::Args`](/silk/docs/std/args/) view,
...
and the current [`std::args::Args`](/silk/docs/std/args/) view.

On `wasm32-wasi`, executable entrypoints remain `fn main () -> int`; use
[`std::args::current()`](/silk/docs/std/args/) (or `std::args::Args.init(std::args::argc(),
std::args::argv)`) to construct the same `Args` value before calling
`FlagSet.parse_args(...)`.

See also:

- [args](/silk/docs/std/args/) (argv helpers)
- [conventions](/silk/docs/std/conventions/) (error/ownership conventions)
- [result](/silk/docs/std/result/) (`Result(T, E)` and `Ok(...)`/`Err(...)` match usage)
