...
implemented in [`std/strings.slk`](https://github.com/oro-computer/silk/blob/master/std/strings.slk) to support early toolchain bring-up; the rest
...
- [literals string](/silk/docs/language/literals-string/) (string semantics: UTF-8 bytes)
- [ext](/silk/docs/language/ext/) (ABI/external-call representation and null-termination rule)
- [buffers](/silk/docs/language/buffers/) (Buffer(T) as the low-level backing store)
- [conventions](/silk/docs/std/conventions/) (UTF-8, allocation, and error conventions)

## Exported API

The following functions exist today in [`std/strings.slk`](https://github.com/oro-computer/silk/blob/master/std/strings.slk) and are available to
import:
...
- `StringBuilder` allocation failure is recoverable:
 ...
 [`std::memory::OutOfMemory?`](/silk/docs/std/memory/) / `Result(...)` and leave the builder unchanged
 on failure (including internal size arithmetic overflow).
- `StringBuilder` conforms to common [`std::interfaces`](/silk/docs/std/interfaces/) protocols:
...
- `String` uses the zero-capacity empty state `{ ptr: 0, cap: 0, len: 0 }`
 ...
 terminator and satisfy [`std::strings::string_storage_well_formed`](/silk/docs/std/strings/), so the
...
- `String.from_chars(chars)` encodes a [`std::arrays::Slice(char)`](/silk/docs/std/arrays/) into UTF-8
...
- `String` also implements [`std::interfaces::{Len,Capacity,IsEmpty,Clear,ReserveAdditional,WriteU8,Serialize(string),TrySerialize(std::memory::OutOfMemory),Parse(std::memory::OutOfMemory),Drop}`](/silk/docs/std/interfaces/).
...
- `owned.try_serialize()` is the canonical fallible owned-string rendering
 path; it clones the current contents into a new [`std::strings::String`](/silk/docs/std/strings/).
- `String.parse(s)` is a standardized alias for `String.from_string(s)`,
 ...
 across the stdlib.
- [`std::strings`](/silk/docs/std/strings/) also owns the reusable verification vocabulary for this
 representation: downstream verified code that wants to talk about owned
 string/path/search-params storage should import
 [`std::strings::string_storage_well_formed(...)`](/silk/docs/std/strings/), not [`std::formal`](/silk/docs/std/formal/).
...
[`std::strings`](/silk/docs/std/strings/) is responsible for:
...
- When converting a `String` to a `string` for FFI, the backing storage must be
 ...
 the external-call contract in [ext](/silk/docs/language/ext/).
...
in [ext](/silk/docs/language/ext/) and [abi libsilk](/silk/docs/compiler/abi-libsilk/).

[`std::strings`](/silk/docs/std/strings/) should provide helpers for common interop patterns:
...
- Formatting integration (shared with [`std::io`](/silk/docs/std/io/)).

Implementation must respect the ownership and lifetime rules from
[ext](/silk/docs/language/ext/) and [buffers](/silk/docs/language/buffers/).
