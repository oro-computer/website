...
section: "std"
order: 111
sourcePath: "std/stream.md"
githubRepo: "oro-computer/silk"
githubRef: "master"
---

# [`std::stream`](/silk/docs/std/stream/)
...
- [concurrency](/silk/docs/language/concurrency/) (tasks, `yield`, structured blocks)
- [sync](/silk/docs/std/sync/) (mutex/condvar/channel; same blocking baseline)
...
In Silk currently, [`std::stream`](/silk/docs/std/stream/) does **not** attach a transformer
...
### File I/O adapters ([`std::fs`](/silk/docs/std/fs/))

[`std::fs`](/silk/docs/std/fs/) provides task-based helpers for piping files into/out of streams:
...
- closes `dst` when [`src`](https://github.com/oro-computer/silk/tree/master/src) ends,
- aborts/cancels on error.

To make piping cooperatively cancellable, use `pipe_to_abortable` with an
[`std::abort_controller::AbortSignalBorrow`](/silk/docs/std/abort_controller/). In the Supported forms, aborts are
