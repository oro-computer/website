...
[`std::os`](/silk/docs/std/os/) provides:
...
- [target metadata](/silk/docs/language/target-metadata/) (the built-in target metadata constants).
...
[`std::os::arch()`](/silk/docs/std/os/) and [`std::os::platform()`](/silk/docs/std/os/) map `ARCH_NAME` and `PLATFORM_NAME`
into enums suitable for [`match`](/silk/wiki/language/flow-match/).
...
[`std::os::uptime()`](/silk/docs/std/os/) returns the current monotonic uptime as a `Duration`.

On the default hosted POSIX runtime, this uses `clock_gettime(CLOCK_MONOTONIC)`
via [`std::temporal`](/silk/docs/std/temporal/).

## CPU information

### `cpu_count`

[`std::os::cpu_count()`](/silk/docs/std/os/) returns the number of logical CPUs available to the
current process (>= 1).

### `cpus`

[`std::os::cpus()`](/silk/docs/std/os/) returns a `Vector(CPU)` with `len == cpu_count()`.
