...
- [concurrency](/silk/docs/language/concurrency/) (language-level `async`/`task`/`yield`/`await` and structured blocks)
- [sync](/silk/docs/std/sync/) (`Mutex`, `Condvar`, and channels)
- [runtime](/silk/docs/std/runtime/) (pluggable runtime layer under [`std::task`](/silk/docs/std/task/))
...
- `sleep_ms(ms)` is implemented by converting `ms` to microseconds and calling
 [`std::runtime::task::sleep_us`](/silk/docs/std/runtime-task/); large sleeps may be performed in chunks.
...
- `sleep_until(deadline)` is a blocking thread operation and is implemented by
 reading [`std::temporal::now_monotonic()`](/silk/docs/std/temporal/) and calling `sleep(deadline - now)`.
 - It returns `Some(SleepUntilFailed{ ... })` when a monotonic clock read fails
 ([`std::temporal::now_monotonic()`](/silk/docs/std/temporal/) returns `Err(...)`).
- `sleep_until_async(deadline)` is an awaitable operation and is implemented by
 reading [`std::temporal::now_monotonic()`](/silk/docs/std/temporal/) and awaiting `sleep_async(deadline - now)`.
 - It returns `Some(SleepUntilFailed{ ... })` when a monotonic clock read fails
 ([`std::temporal::now_monotonic()`](/silk/docs/std/temporal/) returns `Err(...)`).
...
- In the shipped stdlib, [`std::task`](/silk/docs/std/task/) delegates its OS-facing behavior to the
 pluggable runtime interface [`std::runtime::task`](/silk/docs/std/runtime-task/) (which defaults to a POSIX
 backend under [`std::runtime::posix::task`](/silk/docs/std/runtime-posix-task/)).
