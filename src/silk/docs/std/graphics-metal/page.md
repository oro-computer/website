...
section: "std"
order: 141
sourcePath: "std/graphics-metal.md"
...
Source: [`std/graphics/metal.slk`](https://github.com/oro-computer/silk/blob/master/std/graphics/metal.slk)

[`std::graphics::metal`](/silk/docs/std/graphics-metal/) is the opt-in Metal facade for Silk applications. It
...
into [`std::window`](/silk/docs/std/window/) AppKit windows.
...
- On Linux, the lower-level device, command, pipeline, buffer, draw, and
 release APIs are still importable from [`std/graphics/metal`](https://github.com/oro-computer/silk/tree/master/std/graphics/metal); each returns an
...
When a Metal layer is attached to a visible [`std::window`](/silk/docs/std/window/) AppKit window, the
same executable also links `AppKit.framework` through [`std::window`](/silk/docs/std/window/) runtime
requirements.

## Runtime Boundary

[`std::graphics::metal`](/silk/docs/std/graphics-metal/) routes provider calls through
[`std::runtime::graphics::metal`](/silk/docs/std/runtime-graphics-metal/). Application code should not declare Metal
...
[`examples/std_macos_metal_window.slk`](https://github.com/oro-computer/silk/blob/master/examples/std_macos_metal_window.slk) opens a macOS [`std::window`](/silk/docs/std/window/) AppKit
window through the provider-neutral [`std::graphics::window`](/silk/docs/std/graphics-window/) facade, clears a
Metal layer with an animated color, and runs until the user closes the window.

[`examples/std_macos_metal_triangle.slk`](https://github.com/oro-computer/silk/blob/master/examples/std_macos_metal_triangle.slk) uses this module directly. It creates a
