...
[ext](/silk/docs/language/ext/)). For conventional C APIs, prefer `std::ffi::c` scalar
...
- `cstr_copy` produces an **owned** [`std::strings::String`](/silk/docs/std/strings/) copy (heap
 allocation, NUL-terminated).
- [`std::ffi::c_owned::OwnedCStr`](/silk/docs/std/ffi-c_owned/) represents an **owned** C string pointer
...
## Owned pointers ([`std::ffi::c_owned`](/silk/docs/std/ffi-c_owned/))
...
- `OwnedCStr` (owned pointer + drop)
 ...
 [`std::strings::String`](/silk/docs/std/strings/) copy for callers that need the bytes to outlive the
