...
- It uses a simple **handle-based** representation backed by heap-allocated
 state and runtime-provided synchronization primitives ([`std::runtime::sync`](/silk/docs/std/runtime-sync/),
 which defaults to a POSIX/pthread backend in the shipped stdlib).
- All blocking operations block the **current OS thread**.

See also:

- [concurrency](/silk/docs/language/concurrency/) (language-level `task`/`yield`/`await`)
- [task](/silk/docs/std/task/) (task/runtime utilities)

## Thread Safety

[`std::sync`](/silk/docs/std/sync/) is used from OS-thread-backed `task` code. The core pattern is:
