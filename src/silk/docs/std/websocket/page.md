...
[`std::websocket`](/silk/docs/std/websocket/) provides an RFC 6455
WebSocket implementation on top of [`std::net::TCPStream`](/silk/docs/std/networking/) (client + server
handshake, framing, ping/pong, close, fragmentation).

See also:

- [networking](/silk/docs/std/networking/) ([`std::net`](/silk/docs/std/networking/) sockets)
- RFC 6455: The WebSocket Protocol (wire format + handshake)

## Overview

[`std::websocket`](/silk/docs/std/websocket/) is a protocol layer that upgrades an HTTP/1.1 connection to a
WebSocket and then sends/receives WebSocket frames.

Design goals:

- Work for hosted POSIX builds using [`std::net`](/silk/docs/std/networking/).
...
- TLS (`wss://`) integration (use [`std::tls`](/silk/docs/std/tls/) + a future adapter layer).
