...
This module provides list-shaped wrappers
 over the queue core from [`std::queue`](/silk/docs/std/queue/).
...
The implementation is an owning ring-buffer-backed container rather
...
`FixedList(T)` implements the same surface plus [`std::queue::FixedQueue(T)`](/silk/docs/std/queue/).
