...
section: "std"
order: 125
sourcePath: "std/tar.md"
githubRepo: "oro-computer/silk"
githubRef: "master"
---

# [`std::tar`](/silk/docs/std/tar/)

[`std::tar`](/silk/docs/std/tar/) provides a portable,
...
- **Multiple backends**: in-memory bytes, [`std::fs::File`](/silk/docs/std/fs/), and [`std::stream`](/silk/docs/std/stream/).
- **Portability**: tar format logic is pure Silk; the file/IO backends are
 implemented in terms of [`std::fs`](/silk/docs/std/fs/), [`std::io`](/silk/docs/std/io/), and [`std::stream`](/silk/docs/std/stream/).
...
- **Async-friendly**: fd-backed adapters are provided under [`std::tar::async`](/silk/docs/std/tar-async/)
 (built on the hosted event loop and non-blocking [`std::io`](/silk/docs/std/io/)).
...
## [`std::tar::async`](/silk/docs/std/tar-async/) (fd adapters)

[`std::tar::async`](/silk/docs/std/tar-async/) provides `async fn` adapters for fd-backed reading and writing
of tar archives. These are intended for event-loop-based programs:

- issues async reads/writes via the hosted async runtime ([`std::runtime::io`](/silk/docs/std/runtime-io/)),
...
Note: the implementations live in [`std/tar.slk`](https://github.com/oro-computer/silk/blob/master/std/tar.slk); [`std/tar/async.slk`](https://github.com/oro-computer/silk/blob/master/std/tar/async.slk) is a thin
