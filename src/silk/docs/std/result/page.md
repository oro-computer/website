...
The language’s error model is explicit and typed ([errors](/silk/docs/language/errors/)).
[`std::result`](/silk/docs/std/result/) standardizes the common “success or error” return shape so that
APIs across `std::` compose cleanly.

When the standard library is enabled (the default), `Result` is available
without explicit imports via the std prelude module [`std::runtime::globals`](/silk/docs/std/runtime-globals/).
Import [`std::result`](/silk/docs/std/result/) only when you need other exports from the module.
...
- `Result` does not provide aborting unwrap helpers; use `unwrap()` /
 `ok_value()` / `err_value()` (or a [`match`](/silk/wiki/language/flow-match/)) to recover the payload.
...
- Callback-based combinators (`map`, `map_err`, `and_then`, `or_else`) accept
 ...
 captures are restricted (see [memory model](/silk/docs/language/memory-model/)).
...
- [`match`](/silk/wiki/language/flow-match/) supports a shorthand for `Result` destructuring:
...
Statement-form [`match`](/silk/wiki/language/flow-match/) also permits a single handled side for `Result`:
...
Payload-aware recovery still uses `unwrap_or_else`, [`match`](/silk/wiki/language/flow-match/), or `if let Err(...)`
...
explicit [`match`](/silk/wiki/language/flow-match/).
