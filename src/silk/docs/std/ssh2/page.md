...
section: "std"
order: 114
sourcePath: "std/ssh2.md"
githubRepo: "oro-computer/silk"
githubRef: "master"
---

# [`std::ssh2`](/silk/docs/std/ssh2/)

[`std::ssh2`](/silk/docs/std/ssh2/) provides the current
...
[`std::ssh`](/silk/docs/std/ssh/), which forwards this same surface through a compatibility facade.
Use [`std::ssh2`](/silk/docs/std/ssh2/) when you specifically want to refer to the concrete
...
When a program imports [`std::ssh2`](/silk/docs/std/ssh2/) (or the compatibility facade [`std::ssh`](/silk/docs/std/ssh/)),
`silk build` automatically links the target-matched built-in `libssh2.a`
archive from:

- repo builds: [`vendor/lib/<target-layout>/libssh2.a`](https://github.com/oro-computer/silk/blob/master/vendor/lib/<target-layout>/libssh2.a)
...
so [`std::ssh2`](/silk/docs/std/ssh2/) does not require system OpenSSL headers/libraries or a system
...
The [`std::ssh2`](/silk/docs/std/ssh2/) API uses [`std::result::Result(T, E)`](/silk/docs/std/result/) and a stable `SSH2Failed`
...
[`std::ssh2`](/silk/docs/std/ssh2/) expresses byte-oriented inputs and outputs using the standard packed
byte types:

- [`std::arrays::ByteSlice`](/silk/docs/std/arrays/) — non-owning `{ ptr, len }` byte view.
- [`std::buffer::BufferU8`](/silk/docs/std/buffer/) — owning `{ ptr, cap, len }` packed byte buffer.
...
The current [`std::ssh2`](/silk/docs/std/ssh2/) surface currently lives in a single module
[`std/ssh2.slk`](https://github.com/oro-computer/silk/blob/master/std/ssh2.slk) (`module std::ssh2;`) and provides:
