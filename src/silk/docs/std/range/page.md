...
[`std::range`](/silk/docs/std/range/) provides a boxed, method-bearing representation of the built-in
`range` primitive.
...
- The Supported forms is one-way: it builds primitive `range` values from the
 ...
 primitive `range` back into [`std::range::Range`](/silk/docs/std/range/).
