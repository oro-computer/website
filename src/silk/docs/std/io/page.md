...
fd-backed `BufferedWriter`, and a minimal [`std::io::async`](/silk/docs/std/io-async/) subset are
implemented in [`std/io.slk`](https://github.com/oro-computer/silk/blob/master/std/io.slk) via [`std::runtime::io`](/silk/docs/std/runtime-io/).

[`std::io`](/silk/docs/std/io/) provides console and basic stream I/O.

Hosted baseline: POSIX file descriptors and blocking I/O, with a minimal async
subset ([`std::io::async`](/silk/docs/std/io-async/)) backed by the hosted runtime on supported POSIX
...
- [strings](/silk/docs/std/strings/) (formatting targets and string building)
- [fmt](/silk/docs/std/fmt/) (format string syntax)
- [conventions](/silk/docs/std/conventions/) (error conventions)
...
[`std::runtime::io::write`](/silk/docs/std/runtime-io/)), and a minimal async wrapper layer in
[`std::io::async`](/silk/docs/std/io-async/):
...
- `print`/`println` accept Zig-`std.fmt`-style format strings (see
 [fmt](/silk/docs/std/fmt/)) and a variable number of [`std::fmt::Arg`](/silk/docs/std/fmt/) arguments (within
...
- For ergonomics, [`std::fmt::Arg`](/silk/docs/std/fmt/) opts into the compiler’s implicit
 call-argument coercion mechanism (see [types](/silk/docs/language/types/)). This allows
 ...
 implementing [`std::interfaces::Serialize(string)`](/silk/docs/std/interfaces/) also participate in this
...
- [`std::strings::String`](/silk/docs/std/strings/) also satisfies ordinary borrowed `string`
...
- Executable outputs import external libc symbols. On `linux/x86_64`, `silk`
 ...
 a manual libc `--needed` entry is not required for typical hosted [`std::io`](/silk/docs/std/io/)
 use.
...
- [`std::io::async`](/silk/docs/std/io-async/) provides small async wrappers (`read`/`write`) on top of
 [`std::runtime::io::{read_async,write_async}`](/silk/docs/std/runtime-io/). On `linux/*` these are backed
 ...
 [`std::abort_controller::AbortSignalBorrow`](/silk/docs/std/abort_controller/) and return `IOErrorKind::Aborted`
 when cancelled.
...
- [`std::io::stream`](/silk/docs/std/io-stream/) provides task-based adapters that connect POSIX/WASI file
 descriptors (`fd`) with [`std::stream`](/silk/docs/std/stream/) (`ReadableStream` / `WritableStream`):
 - [`std::io::stream::pipe_fd_to_stream`](/silk/docs/std/io-stream/) / `pipe_fd_to_stream_abortable`
 - [`std::io::stream::pipe_stream_to_fd`](/silk/docs/std/io-stream/) / `pipe_stream_to_fd_abortable`
...
[`std::io`](/silk/docs/std/io/) is responsible for:

- Standard input, output, and error streams.
- Simple printing and formatted output APIs.
- Minimal fd-based async wrappers in [`std::io::async`](/silk/docs/std/io-async/).
...
key point is that [`std::fs`](/silk/docs/std/fs/) and [`std::net`](/silk/docs/std/networking/) can reuse the same I/O traits.
...
- Broader async I/O surface beyond the shipped [`std::io::async`](/silk/docs/std/io-async/) wrappers:
