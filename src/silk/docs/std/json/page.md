...
[`std::json`](/silk/docs/std/json/) provides an RFC 8259-style JSON parser plus deterministic JSON
...
- The internal DOM storage now uses a private
 [`std::json::dom_storage_well_formed(...)`](/silk/docs/std/json/) theory defined inside
 [`std/json.slk`](https://github.com/oro-computer/silk/blob/master/std/json.slk), and the public document accessors attach that local contract
 directly.
