...
[`std/args.slk`](https://github.com/oro-computer/silk/blob/master/std/args.slk) to make early programs ergonomic while `string[]` parameters and
richer slice/iterator features are still future work.

[`std::args`](/silk/docs/std/args/) currently supports two executable argument shapes:
...
pointer list (`char**` in C). For `wasm32-wasi`, [`std::args`](/silk/docs/std/args/) exposes helpers
...
- [cli silk](/silk/docs/compiler/cli-silk/) (entrypoint selection rules)
- [ext](/silk/docs/language/ext/) (string ABI `{ ptr, len }` and C-string lowering rules)

## Exported API
The following items are implemented in [`std/args.slk`](https://github.com/oro-computer/silk/blob/master/std/args.slk):
...
This intrinsic is intended only for stdlib bring-up (in particular [`std::args`](/silk/docs/std/args/)
