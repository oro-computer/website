...
[`std::formal`](/silk/docs/std/formal/) is intentionally about **base facts**, not every theory the
...
See [formal verification](/silk/docs/language/formal-verification/) for the Formal Silk execution model
and theory semantics.

When [`std::formal`](/silk/docs/std/formal/) is built and packaged as distributable Silk surface, its
exported theories participate in the same Formal Silk export-bundle mechanism
as any other package: the source declarations remain authoritative for import
and verification, while the generated `manifest.json` / `bundle.smt2` payload
provides machine-readable solver material for tooling and package distribution.
...
## What lives outside [`std::formal`](/silk/docs/std/formal/)
...
- [`std::semver::{semver_core_eq,semver_core_ge,semver_core_gt,semver_core_lt,semver_core_le}`](/silk/docs/std/semver/)
 - reusable SemVer core-triplet ordering vocabulary
- [`std::uuid::{uuid_nil_words,uuid_version_field_is,uuid_rfc4122_variant,uuid_rfc4122_version_is}`](/silk/docs/std/uuid/)
 - UUID layout vocabulary
- [`std::runtime::build::{build_kind_is,build_mode_is,requires_debug_mode,requires_release_mode,requires_executable_kind,requires_object_kind,requires_static_kind,requires_shared_kind,build_version_at_least}`](/silk/docs/std/runtime-build/)
 - build-metadata verification vocabulary
- [`std::test::requires_test_mode()`](/silk/docs/std/test/)
 - test-only helper contract surface
- [`std::json`](/silk/docs/std/json/) / [`std::toml`](/silk/docs/std/toml/)
