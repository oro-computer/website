...
The implementation uses a circular heap buffer with a logical front
index:
...
- [`std::interfaces::Iterator(T)`](/silk/docs/std/interfaces/) via destructive `next()` semantics

Fixed queues implement the same surface plus [`std::queue::FixedQueue(T)`](/silk/docs/std/queue/).
