...
section: "std"
order: 104
sourcePath: "std/uuid.md"
githubRepo: "oro-computer/silk"
githubRef: "master"
---

# [`std::uuid`](/silk/docs/std/uuid/)

[`std::uuid`](/silk/docs/std/uuid/) provides a robust UUID/ULID-like
...
- [`std::uuid::parse(s: string) -> std::uuid::ParseResult`](/silk/docs/std/uuid/) returns
...
- `UUID.parse(s: string) -> std::uuid::ParseResult` is the standardized
 receiverless parse form via [`std::interfaces::Parse(ParseError)`](/silk/docs/std/interfaces/).
- `UUID.try_serialize() -> Result(String, OutOfMemory)` is the standardized
 fallible owned-string rendering path via
 [`std::interfaces::TrySerialize(std::memory::OutOfMemory)`](/silk/docs/std/interfaces/).
...
- `UUID.from_u64s(hi, lo)` guarantees the stored words are exactly `hi` and
 `lo`.
- `UUID.nil()` proves the reusable [`std::uuid::uuid_nil_words(...)`](/silk/docs/std/uuid/) theory.
...
- [`std::uuid::{namespace_dns,namespace_url,namespace_oid,namespace_x500}`](/silk/docs/std/uuid/) prove
 [`std::uuid::uuid_rfc4122_version_is(..., 1)`](/silk/docs/std/uuid/).
- [`std::uuid::v4_from_u64s`](/silk/docs/std/uuid/) proves
 [`std::uuid::uuid_rfc4122_version_is(..., 4)`](/silk/docs/std/uuid/).
- [`std::uuid::v8_from_u64s`](/silk/docs/std/uuid/) proves
 [`std::uuid::uuid_rfc4122_version_is(..., 8)`](/silk/docs/std/uuid/).
...
- So [`std::uuid::{v1,v3,v5,v6,v7}`](/silk/docs/std/uuid/) are still fully implemented and tested, but
...
- [`std::uuid::{v1,v3,v5,v6,v7}`](/silk/docs/std/uuid/) return [`std::uuid::UUIDResult`](/silk/docs/std/uuid/) (`Result(UUID, UUIDFailed)`).
- [`std::uuid::timestamp_100ns_from_unix_ns`](/silk/docs/std/uuid/) returns `Result(u64, UUIDFailed)` and
 may fail with `Overflow`.
- [`std::uuid::random::*`](/silk/docs/std/uuid-random/) returns [`std::uuid::UUIDResult`](/silk/docs/std/uuid/) and may fail due to
 `InitFailed` (libsodium) or `NoTime` (missing clock).

[`std::uuid::random`](/silk/docs/std/uuid-random/) also provides `v1_now` / `v6_now` / `v7_now` using
[`std::runtime::time::unix_now_ns`](/silk/docs/std/runtime-time/) / `unix_now_ms` so callers can generate UUIDs
without passing explicit timestamps.

Planned follow-ups:

- richer formatting options (uppercase, simple hex, braced form, URN form),
- UUIDv2 (DCE Security) if/when [`std::process`](/silk/docs/std/process/) exposes stable UID/GID APIs.
