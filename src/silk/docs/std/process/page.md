...
[`std::process`](/silk/docs/std/process/) provides access to process-level operations that are not tied to
...
implemented on top of the pluggable [`std::runtime::process`](/silk/docs/std/runtime-process/) interface. WASI
...
[`std::process::effective_user_id()`](/silk/docs/std/process/) returns the operating system's effective
...
[`std::fs::Stats.uid`](/silk/docs/std/fs/) before accepting a security-sensitive path.
...
## Child processes ([`std::process::child`](/silk/docs/std/process-child/))

[`std::process::child`](/silk/docs/std/process-child/) provides hosted child-process execution (spawn/wait/kill)
and high-level output capture.

For convenience, [`std::process`](/silk/docs/std/process/) re-exports the high-level [`std::process::child`](/silk/docs/std/process-child/)
...
See the [`std::process::child`](/silk/docs/std/process-child/) source ([`std/process/child.slk`](https://github.com/oro-computer/silk/blob/master/std/process/child.slk)) for the exact,
...
### Child stdio pipes and [`std::stream`](/silk/docs/std/stream/)

When a child is spawned with piped stdio (`Stdio::Pipe`), [`std::process::child`](/silk/docs/std/process-child/)
exposes the pipes as POSIX file descriptors (`Child.take_stdin()` /
`take_stdout()` / `take_stderr()`).

To treat those file descriptors as [`std::stream`](/silk/docs/std/stream/) byte streams, use the
task-based adapters in [`std::io::stream`](/silk/docs/std/io-stream/):

- [`std::io::stream::pipe_fd_to_stream`](/silk/docs/std/io-stream/) / `pipe_fd_to_stream_abortable` (fd → `WritableStream`)
- [`std::io::stream::pipe_stream_to_fd`](/silk/docs/std/io-stream/) / `pipe_stream_to_fd_abortable` (`ReadableStream` → fd)
...
[`std::process::getcwd()`](/silk/docs/std/process/) returns the current working directory as an owned
...
[`std::process::executable_path()`](/silk/docs/std/process/) returns an owned path to the current
executable.
...
`argv[0]`. Use [`std::env::executable_path_from_args(argc, argv)`](/silk/docs/std/env/) when the
borrowed launcher-provided argv string is the desired zero-copy representation.

## `chdir`

[`std::process::chdir(path)`](/silk/docs/std/process/) changes the process working directory.
...
- `chdir` does not update environment variables like `PWD`. Use
 [`std::process::getcwd()`](/silk/docs/std/process/) to query the real current directory.
...
- **WASI (Preview 1)**: `getcwd` and `chdir` are implemented via a virtual
 ...
 and the OS-queried
 executable path is unavailable; use [`std::args::current().get(0)`](/silk/docs/std/args/) or
 [`std::env::executable_path_from_args`](/silk/docs/std/env/) when argv is available.
 [`std::process::child`](/silk/docs/std/process-child/) operations remain unsupported.
