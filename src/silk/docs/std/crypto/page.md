...
section: "std"
order: 112
sourcePath: "std/crypto.md"
githubRepo: "oro-computer/silk"
githubRef: "master"
---

# [`std::crypto`](/silk/docs/std/crypto/)

[`std::crypto`](/silk/docs/std/crypto/) provides
...
- a cohesive, ergonomic [`std::crypto`](/silk/docs/std/crypto/) API surface that is suitable for Silk
 programs,
...
On Apple targets with the `auto` or `platform` provider, [`std::crypto::init`](/silk/docs/std/crypto/),
[`std::crypto::memzero`](/silk/docs/std/crypto/), [`std::crypto::equal`](/silk/docs/std/crypto/), and [`std::crypto::random`](/silk/docs/std/crypto-random/) route
...
- the repo checkout: [`vendor/lib/<target-layout>/`](https://github.com/oro-computer/silk/tree/master/vendor/lib/<target-layout>/), or
...
([`std::crypto::hash`](/silk/docs/std/crypto-hash/), [`std::crypto::aead`](/silk/docs/std/crypto-aead/), [`std::crypto::secretbox`](/silk/docs/std/crypto-secretbox/),
[`std::crypto::box`](/silk/docs/std/crypto-box/), and [`std::crypto::sign`](/silk/docs/std/crypto-sign/)) fall back to the built-in
libsodium archive while shared [`std::crypto`](/silk/docs/std/crypto/) core/random helpers still use
...
The current [`std::vector`](/silk/docs/std/vector/)/[`std::buffer`](/silk/docs/std/buffer/) element model stores each
generic element in an 8-byte slot (even for `u8`), so [`std::vector::Vector(u8)`](/silk/docs/std/vector/)
...
- [`std::arrays::ByteSlice`](/silk/docs/std/arrays/) — a non-owning `{ ptr, len }` view over packed bytes
 (`len` in bytes).
- [`std::buffer::BufferU8`](/silk/docs/std/buffer/) — an owning `{ ptr, cap, len }` packed byte buffer
 (`cap`/`len` in bytes).

[`std::crypto`](/silk/docs/std/crypto/) expresses byte-oriented inputs and outputs in terms of these
...
The initial [`std::crypto`](/silk/docs/std/crypto/) module is organized as:

- [`std::crypto`](/silk/docs/std/crypto/) (core helpers and provider init)
- [`std::crypto::random`](/silk/docs/std/crypto-random/) (CSPRNG; Security on Apple platform, libsodium on
 builtin)
- [`std::crypto::hash`](/silk/docs/std/crypto-hash/) (variable-length BLAKE2b, keyed BLAKE2b, and fixed
 32-byte SHA-256 hashing)
- [`std::crypto::aead`](/silk/docs/std/crypto-aead/) (AEAD: ChaCha20-Poly1305 IETF and XChaCha20-Poly1305 IETF)
- [`std::crypto::secretbox`](/silk/docs/std/crypto-secretbox/) (secret-key authenticated encryption)
- [`std::crypto::box`](/silk/docs/std/crypto-box/) (public-key authenticated encryption)
- [`std::crypto::sign`](/silk/docs/std/crypto-sign/) (signatures)
...
- functions return recoverable error values:
 ...
 - [`std::result::Result(T, ErrorType)`](/silk/docs/std/result/) where `Ok(T)` is success and `Err(ErrorType)` is failure
 (use `Result(bool, ErrorType)` for fallible predicates).

### Hashing ([`std::crypto::hash`](/silk/docs/std/crypto-hash/))
...
See [crypto hash](/silk/docs/std/crypto-hash/) for exact signatures and
output ownership.

### AEAD ([`std::crypto::aead`](/silk/docs/std/crypto-aead/))
