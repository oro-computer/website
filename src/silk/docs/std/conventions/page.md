...
- Packages use `std::area` naming ([`std::strings`](/silk/docs/std/strings/), [`std::fs`](/silk/docs/std/fs/), [`std::net`](/silk/docs/std/networking/)).
...
(`/** ... */` or `/// ...`) per [doc comments](/silk/docs/language/doc-comments/).
...
- The built-in `string` type is a UTF-8 byte sequence represented as a
 `{ ptr, len }` pair (see [literals string](/silk/docs/language/literals-string/) and
 [ext](/silk/docs/language/ext/)).
...
- If a type’s primary constructor requires configuration (capacity, hash function pointers, etc.), provide a convenience constructor that uses a sensible default:
 - for capacity-driven containers: `empty()` and a parameterized constructor (`init(cap)` in the Supported forms; consider `with_capacity(cap)` long-term),
...
- Each owning type must document:
 - which operation releases ownership (`close`, `drop`, `deinit`, etc.),
 - whether repeated cleanup is valid,
 - and which states are inert / invalid / empty.
- Cleanup operations should leave the value in an inert state after releasing
 ownership so accidental double-drop and post-close use are easier to detect
 and avoid.
- Async or split-ownership APIs must not rely on vague caller discipline:
 - either preserve the required lifetime structurally (duplication,
 ref-counting, owned promise state, etc.),
 - or document and test the exact lifetime rule the caller must obey.
- When an owning/resource API is added or changed, land release-path regression
 coverage for:
 - the success path,
 - and at least one invalid / error / no-op / early-return cleanup path.
- When the same behavior is visible through the public C ABI, add C99 coverage
 as well as Zig / Silk coverage.

## Errors

Silk supports both typed errors ([typed errors](/silk/docs/language/typed-errors/)) and
recoverable error values (`T?` and [`std::result::Result(T, E)`](/silk/docs/std/result/)). Public `std::`
...
- Use a result type when callers need to distinguish multiple error causes.
 The design target is [`std::result::Result(T, E)`](/silk/docs/std/result/) (see [result](/silk/docs/std/result/)),
...
- Keep OS/runtime-specific error mechanisms (such as POSIX `errno`) out of the
 ...
 top-level `std::` module and confine the platform details to [`std::runtime`](/silk/docs/std/runtime/).
...
- Prefer **immutable value types** (no interior mutation) for data that will be
 shared across tasks.
- For shared mutable state, require explicit synchronization via [`std::sync`](/silk/docs/std/sync/)
...
- Cancellation must be explicit:
 - abortable operations should accept [`std::abort_controller::AbortSignalBorrow`](/silk/docs/std/abort_controller/)
...
- Prefer reusable theories from [`std::formal`](/silk/docs/std/formal/) (for example `slice_well_formed`
...
- Contracts must reflect real runtime invariants (avoid over-strong
 ...
 on preconditions.
- See [formal verification](/silk/docs/language/formal-verification/) for the Formal Silk model. Note
