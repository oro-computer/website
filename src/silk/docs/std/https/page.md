...
[`std::https`](/silk/docs/std/https/) provides a
small HTTPS client/server connection API on top of [`std::tls`](/silk/docs/std/tls/) (currently the
built-in mbedTLS provider) and [`std::net::TCPStream`](/silk/docs/std/networking/), plus URL and stream
convenience helpers.

See also:

- [http](/silk/docs/std/http/) ([`std::http`](/silk/docs/std/http/) message parsing/serialization)
- [tls](/silk/docs/std/tls/) ([`std::tls`](/silk/docs/std/tls/) TLS sessions and transport integration)
- [stream](/silk/docs/std/stream/) ([`std::stream`](/silk/docs/std/stream/) byte streams)
- [url](/silk/docs/std/url/) ([`std::url`](/silk/docs/std/url/) parsing and serialization)
- RFC 2818 (HTTP over TLS)

## Scope
Provider note: because this module depends on [`std::tls`](/silk/docs/std/tls/), default Apple `auto`
builds currently fall back to the built-in mbedTLS provider for TLS. Explicit
`platform` builds reject [`std::https`](/silk/docs/std/https/) until the Network/Security-backed TLS
surface is specified and implemented.



- Blocking TLS handshake using [`std::tls::Session`](/silk/docs/std/tls/).
- Authenticated HTTPS client sessions via `Connection.connect_host(...)` using a
 system CA bundle + hostname verification.
- HTTPS request/response I/O using the same message model as [`std::http`](/silk/docs/std/http/).
...
- Header maps from [`std::http::HeadersMap`](/silk/docs/std/http/) for custom request headers.
...
- Fully incremental streaming bodies. The stream helpers bridge
 [`std::stream`](/silk/docs/std/stream/) to the current owned-message implementation: request streams
...
- `connect_host(...)` / `connect_host_v6(...)` perform certificate chain
 ...
 calling [`std::tls::Session.set_hostname(...)`](/silk/docs/std/tls/) before the handshake.
...
- `request_url(...)` accepts absolute `https://...` URLs. It parses with
 [`std::url`](/silk/docs/std/url/), uses port `443` when the URL has no explicit non-default port,
 resolves/connects with [`std::net::resolve_host(...)`](/silk/docs/std/networking/), establishes a verified
...
- Header-aware helpers consume [`std::http::HeadersMap`](/silk/docs/std/http/), use the same validation
 and constructor semantics documented by [`std::http`](/silk/docs/std/http/), and keep `Host`,
...
- Stream helpers map [`std::stream`](/silk/docs/std/stream/) failures to `ERR_STREAM`, except allocation
