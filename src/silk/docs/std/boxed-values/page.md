...
section: "std"
order: 231
sourcePath: "std/boxed-values.md"
...
- [`std::strings::String`](/silk/docs/std/strings/) boxes borrowed `string` as an owned mutable value.
- [`std::number::Number`](/silk/docs/std/number/) provides an inline tagged numeric box with parsing,
 formatting, predicates, exact integer extraction helpers, and conservative
 mixed-kind equality.
- [`std::boolean::Boolean`](/silk/docs/std/boolean/) boxes `bool` without allocation.
- [`std::path::Path`](/silk/docs/std/path/) provides a borrowed method-bearing path box alongside the
 existing owned `PathBuf`.
- [`std::range::Range`](/silk/docs/std/range/) provides a stdlib-owned method surface that can emit the
 built-in `range` primitive via `as_range()`.
- [`std::function::Function(F)`](/silk/docs/std/function/) provides a boxed callable holder around an
 existing function-value type.
- [`std::regex::RegExp`](/silk/docs/std/regex/) remains the owned runtime regex wrapper.
- `Optional(T)` already exposes the `Result`-style combinator subset, and
 [`std::optional`](/silk/docs/std/optional/) now provides free-function companions for that built-in
 method surface.

## Box Families

### [`std::number::Number`](/silk/docs/std/number/)
...
### [`std::path::Path`](/silk/docs/std/path/)

`Path` should be the boxed path value.

Design direction:

- The implementation is a borrowed wrapper over `string`, paired with
...
### [`std::boolean::Boolean`](/silk/docs/std/boolean/)
...
### [`std::regex::RegExp`](/silk/docs/std/regex/)
...
[`std::optional`](/silk/docs/std/optional/) provides a stdlib namespace of companion free functions for the
same operations.

### [`std::range::Range`](/silk/docs/std/range/)
...
### [`std::function::Function`](/silk/docs/std/function/)
...
1. Decide whether [`std::range::Range`](/silk/docs/std/range/) also needs a compiler-supported
 decomposition path from primitive `range` back into the stdlib box.
2. Decide whether [`std::function::Function(F)`](/silk/docs/std/function/) should grow arity-specialized
