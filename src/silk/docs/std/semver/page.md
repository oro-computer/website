...
[`std::semver`](/silk/docs/std/semver/) provides a SemVer 2.0.0 parser and
...
- `Version.parse(input: string) -> ParseResult`
 ...
 [`std::interfaces::Parse(ParseError)`](/silk/docs/std/interfaces/).
...
`ParseResult` is [`std::result::Result(Version, ParseError)`](/silk/docs/std/result/).
...
- `Version.cmp` carries verified postconditions that:
 ...
 negative.
- Helper comparators inside [`std::semver`](/silk/docs/std/semver/) (`cmp_u64`, `cmp_lex`, `cmp_ident`,
...
- [`std::semver`](/silk/docs/std/semver/) also exports reusable Formal Silk theories for the
