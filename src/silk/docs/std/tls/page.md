...
[`std::tls`](/silk/docs/std/tls/) provides TLS client/server
primitives for the hosted POSIX baseline using the built-in mbedTLS provider.

The initial goals are:

- a small but usable [`std::tls`](/silk/docs/std/tls/) session API for clients and servers,
- a transport-agnostic I/O model so TLS can be layered over [`std::net::TCPStream`](/silk/docs/std/networking/)
 or custom runtimes ,
...
for [`std::tls`](/silk/docs/std/tls/) because the Network/Security-backed TLS implementation is not
wired yet. Explicit `platform` builds are strict and reject [`std::tls`](/silk/docs/std/tls/) for now
...
The current [`std::tls`](/silk/docs/std/tls/) API uses [`std::result::Result(T, E)`](/silk/docs/std/result/) and a stable
...
[`std::tls`](/silk/docs/std/tls/) treats this as a retryable read condition (surfaced as
...
- `set_bio_mempipe(bio: u64) -> void` — attach a `MemPipe` endpoint context via
 mbedTLS `ssl_set_bio` using [`std::tls::mem_send`](/silk/docs/std/tls/) and [`std::tls::mem_recv`](/silk/docs/std/tls/).
- `set_bio_fd(fd: int) -> void` — attach a hosted POSIX file descriptor as the
 underlying stream transport (for example a [`std::net::TCPStream`](/silk/docs/std/networking/) socket).
...
`Session` implements [`std::interfaces::Drop`](/silk/docs/std/interfaces/) and releases all associated mbedTLS
state on drop.
...
- The current [`std::tls`](/silk/docs/std/tls/) API is intentionally small; higher-level features
