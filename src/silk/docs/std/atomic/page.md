...
[`std::atomic`](/silk/docs/std/atomic/) provides the public Silk API for low-level atomic operations.

Prefer [`std::sync`](/silk/docs/std/sync/) for ordinary synchronization. Use this module when a data
...
The public module delegates to [`std::runtime::atomic`](/silk/docs/std/runtime-atomic/), which delegates to the
