...
section: "usage"
order: 245
sourcePath: "usage/pure-silk-gpu.md"
...
[`std::gpu::last_error()`](/silk/docs/std/gpu/) when a runtime diagnostic is wanted.

The `gpu` expression resolves `fill_u32` and checks its arguments at compile
time, then calls the replaceable [`std::gpu::launch_and_synchronize`](/silk/docs/std/gpu/)
...
[`examples/pure_silk_gpu_ml.slk`](https://github.com/oro-computer/silk/blob/master/examples/pure_silk_gpu_ml.slk) intentionally retains that convention so both
...
older [`std::gpu::device::store_u32_at_global_x`](/silk/docs/std/gpu-device/) remains a concise compatibility
operation. GPU entries may declare up to 32 immutable `u64` parameters, and
[`std::gpu::launch`](/silk/docs/std/gpu/) automatically packs the same number of trailing values. The
...
standalone [`std::gpu::isa`](/silk/docs/std/gpu-isa/) source-kernel path.
