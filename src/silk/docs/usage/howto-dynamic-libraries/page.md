...
section: "usage"
order: 243
sourcePath: "usage/howto-dynamic-libraries.md"
...
Use [`std::dylib`](/silk/docs/std/dylib/) for runtime-selected libraries and symbols. Use `ext` for
...
provides `dlopen` when [`std::dylib`](/silk/docs/std/dylib/) is imported (`libdl.so.2` on glibc,
...
See [`examples/std_dylib_errors.slk`](https://github.com/oro-computer/silk/blob/master/examples/std_dylib_errors.slk) for a runnable error-path example.
...
- Link errors for `silk_rt_dylib_*`: rebuild or reinstall Silk so
 `libsilk_rt.a` contains [`src/silk_rt_dylib.c`](https://github.com/oro-computer/silk/blob/master/src/silk_rt_dylib.c); when using a copied local
