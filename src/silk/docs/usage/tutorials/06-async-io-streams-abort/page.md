...
section: "usage"
order: 238
sourcePath: "usage/tutorials/06-async-io-streams-abort.md"
...
- [`std::io::async`](/silk/docs/std/io-async/) for fd readiness + read/write loops,
- [`std::stream`](/silk/docs/std/stream/) for in-memory byte streams and transform stages,
- [`std::abort_controller`](/silk/docs/std/abort_controller/) for cooperative cancellation.
...
- [io](/silk/docs/std/io/) ([`std::io::async`](/silk/docs/std/io-async/) and [`std::io::stream`](/silk/docs/std/io-stream/))

## 1) AbortController: a single cancellation story

[`std::abort_controller`](/silk/docs/std/abort_controller/) provides WHATWG-style `AbortController` /
...
## 2) High-quality async I/O ([`std::io::async`](/silk/docs/std/io-async/))

[`std::io::async`](/silk/docs/std/io-async/) is a small layer over the hosted event loop:

1. wait for fd readability/writability ([`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/)),
2. call the existing synchronous [`std::io::{read,write}`](/silk/docs/std/io/) functions,
...
- Aborts are cooperative, but the details depend on the layer:
 - [`std::runtime::event_loop::fd_wait_readable_abortable`](/silk/docs/std/runtime-event_loop/) can interrupt an
 in-flight readiness wait when `AbortSignalBorrow.wait_fd()` is available.
 - [`std::io::async::{read_abortable,write_abortable}`](/silk/docs/std/io-async/) still observe aborts
...
- This example intentionally avoids `task fn` threads. In the current hosted
 ...
 not reliable yet. Keep [`std::io::async`](/silk/docs/std/io-async/)-based I/O pipelines single-threaded
 for now, and use `task fn` for thread-based parallelism in separate examples
 (see section 1).

## 3) Stream pipelines ([`std::stream`](/silk/docs/std/stream/)) + fd adapters

[`std::stream`](/silk/docs/std/stream/) provides byte streams that are easy to compose:
...
- [`std::io::stream`](/silk/docs/std/io-stream/) pipes fd ↔ stream (`ReadableStream` / `WritableStream`).
...
- For network I/O, start with [networking](/silk/docs/std/networking/)
 ([`std::net::stream`](/silk/docs/std/net-stream/) provides `TcpStream` ↔ stream adapters).
- For low-level readiness waits and manual executor driving, read
 [runtime event loop](/silk/docs/std/runtime/).
