...
section: "usage"
order: 246
sourcePath: "usage/pure-silk-gpu-ml.md"
...
[`examples/pure_silk_gpu_ml.slk`](https://github.com/oro-computer/silk/blob/master/examples/pure_silk_gpu_ml.slk) is a small but real supervised-learning
...
[`std::buffer::BufferU8`](/silk/docs/std/buffer/). Its CPU `centroid` routine loops over the supplied
...
[`std::gpu::launch`](/silk/docs/std/gpu/) packs the two device addresses and learned threshold as
...
The source uses the public [`std::buffer::BufferU8`](/silk/docs/std/buffer/) surface and four byte pushes
