...
section: "guides"
order: 6
sourcePath: "guides/practical-logger-module.md"
...
Silk API in [`src/lib.slk`](https://github.com/oro-computer/silk/blob/master/src/lib.slk), then adds [`src/c_api.slk`](https://github.com/oro-computer/silk/blob/master/src/c_api.slk) for the narrow ABI wrapper.
...
[`src/lib.slk`](https://github.com/oro-computer/silk/blob/master/src/lib.slk) is the normal Silk API. [`src/c_api.slk`](https://github.com/oro-computer/silk/blob/master/src/c_api.slk) is the C/ABI edge.
...
target enters through [`src/c_api.slk`](https://github.com/oro-computer/silk/blob/master/src/c_api.slk) because that artifact is meant for ABI
consumers; ordinary Silk consumers import the default module with `"logger"`.

## Implementation Module

[`src/lib.slk`](https://github.com/oro-computer/silk/blob/master/src/lib.slk) defines the reusable logging API:
...
[`examples/main.slk`](https://github.com/oro-computer/silk/blob/master/examples/main.slk) imports the package's default module namespace and handles parse/write
outcomes explicitly:
...
[`tests/logger_test.slk`](https://github.com/oro-computer/silk/blob/master/tests/logger_test.slk) exercises parsing and filtering:
...
- [`std/io`](https://github.com/oro-computer/silk/tree/master/std/io) and [`std/test`](https://github.com/oro-computer/silk/tree/master/std/test) load stdlib modules from the configured std root.
- `Result` is a std-prelude global from [`std::runtime::globals`](/silk/docs/std/runtime-globals/), so it does not
 need an import in user-space modules.
- `logger` resolves through the dependency key or package root named `logger`;
 by convention it loads the package's [`src/lib.slk`](https://github.com/oro-computer/silk/blob/master/src/lib.slk) module.
...
[`src/c_api.slk`](https://github.com/oro-computer/silk/blob/master/src/c_api.slk) exposes a C-friendly wrapper:
...
- [Modules, Packages, and Publication](/silk/docs/guides/modules-and-packages/)
- [Packages, Imports, and Exports](/silk/docs/language/packages-imports-exports/)
