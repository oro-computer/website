...
- Runtime `let`/`var` bindings and compile-time `const` bindings must have an initializer ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2015`).
...
- `const` initializers must be compile-time evaluable ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2041`); in the Supported forms this is restricted to scalar expressions and calls to `const fn` functions (still no `/` or `%`), plus string literals / `const` string aliases.
...
- A small concurrency subset is implemented (`Task(T)` / `Promise(T)` plus `yield`/`await`; see [`Concurrency`](/silk/docs/language/concurrency/)).
...
See [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/) and `Interfaces` for details.
...
- Allocation:
 - `new` uses the active region inside `with` (see [`Regions`](/silk/docs/language/regions/)).
...
See [`Concurrency`](/silk/docs/language/concurrency/) for deeper semantics.
...
- [`Formal Grammar Spec`](/silk/docs/language/grammar/) (the exact grammar the parser accepts),
...
- and [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/) (error codes for unsupported forms).
...
- [`Formal Grammar Spec`](/silk/docs/language/grammar/) for syntax,
- `the implementation status` for current end-to-end support,
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) for working example programs.
...
Doc comments (tooling-only; see [`Silkdoc (Documentation Comments)`](/silk/docs/language/doc-comments/)):
...
Top-level ordering is enforced (see [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/)):
...
See [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/) for the full import/export model.
...
- Only `let mut`/`var` bindings are assignable lvalues (see
 [`Mutability`](/silk/docs/language/mutability/) and `Operators`).
...
`async fn` / `task fn` / `async task fn` (handles; see [`Concurrency`](/silk/docs/language/concurrency/)):
...
call site (see [`Mutability`](/silk/docs/language/mutability/)):
...
Varargs (final parameter prefixed by `...`; see [`Varargs (Variable Arguments)`](/silk/docs/language/varargs/)):
...
and [`Memory Model (Stack, Heap, and Moves)`](/silk/docs/language/memory-model/).
...
Attach methods with `impl` (see [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)):
...
Single inheritance (current surface is implemented; see [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)):
...
#### 4.6 Enums: `enum` + [`match`](/silk/wiki/language/flow-match/) expression
...
#### 4.8 Typed errors: `error`, `panic`, `T | ErrorType...`, [`match`](/silk/wiki/language/flow-match/) statement, `?`
...
Handling typed errors uses the [`match`](/silk/wiki/language/flow-match/) **statement** form:
...
[`Varargs (Variable Arguments)`](/silk/docs/language/varargs/) and `External Declarations (`ext`)`.

See also: `C99 ABI and `libsilk.a`` (C ABI) and [`include/silk/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk/silk.h).
...
Optional [`match`](/silk/wiki/language/flow-match/) expressions are the explicit form of optional consumption:
...
See [`Generics (Monomorphized)`](/silk/docs/language/generics/) for Supported forms limits (notably `E2016` for
const parameters/arguments and generic functions).

### 6. Statements (Inside Blocks)

The statement grammar is summarized in [`Formal Grammar Spec`](/silk/docs/language/grammar/) and detailed
...
#### [`loop`](/silk/wiki/language/flow-loop/), [`while`](/silk/wiki/language/flow-while/), `for`
...
#### [`match`](/silk/wiki/language/flow-match/) statement (typed errors)
...
productions, see [`Formal Grammar Spec`](/silk/docs/language/grammar/).
...
- Syntax: [`Formal Grammar Spec`](/silk/docs/language/grammar/)
- Types: `Types`, [`Generics (Monomorphized)`](/silk/docs/language/generics/)
- Operators: `Operators`
- Flow control: [`Flow Control Overview`](/silk/docs/language/flow-overview/) and the relevant chapters of this specification
- Modules/imports/exports: [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/)
- Optionals: [`Optional`](/silk/docs/language/optional/)
- Typed errors: `Typed Errors (`error`, `panic`, and `T | ErrorType...`)`
- Concurrency: [`Concurrency`](/silk/docs/language/concurrency/)
...
- Whitespace and comments (trivia):
 ...
 documentation generation as specified in [`Silkdoc (Documentation Comments)`](/silk/docs/language/doc-comments/).
- Identifiers:
 ...
 file imports: `from`,
 - control flow: `if`, `else`, [`loop`](/silk/wiki/language/flow-loop/), [`while`](/silk/wiki/language/flow-while/), `for`, `in`, [`match`](/silk/wiki/language/flow-match/), `return`, `panic`, `break`, `continue`, `assert`, `await`, `yield`,
 ...
 qualified-name segments like [`std::test`](/silk/docs/std/test/), function/method names, and member
...
- Literals:
 ...
 - `/pattern/flags` (see [`Regular Expression Literals`](/silk/docs/language/literals-regexp/)).
...
- Packages and imports:
 ...
 [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/). The grammar in this file mirrors
 ...
 **final**,
 ...
 - strings starting with [`std/`](https://github.com/oro-computer/silk/tree/master/std/) are treated as std package specifiers and
 ...
 the checker (see [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2015`).

 Additionally, `const` initializers must be compile-time evaluable; otherwise
 the compiler reports an error (see [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2041`).
...
- Types (Supported forms):
 ...
 [`Memory Model (Stack, Heap, and Moves)`](/silk/docs/language/memory-model/)).
...
- Statements (Supported forms):
 ...
 Region declarations and `with` blocks are specified in [`Regions`](/silk/docs/language/regions/).

 [`match`](/silk/wiki/language/flow-match/) is implemented in two separate forms:

 - [`match`](/silk/wiki/language/flow-match/) as an expression (arms are expressions; see `MatchExpr` below),
 - [`match`](/silk/wiki/language/flow-match/) as a statement (arms are blocks), used for typed errors as
 specified in `Typed Errors (`error`, `panic`, and `T | ErrorType...`)`.

 In the Supported forms, the [`match`](/silk/wiki/language/flow-match/) statement form is restricted to a
...
[`match`](/silk/wiki/language/flow-match/) expression form as part of the implemented optional subset, but other
...
- Supported end-to-end: primitives, nominal `struct` types, optionals (`T?`),
 ...
 in the backend (including array literals, indexing reads, and
...
- Parsed but rejected by the current checker: const parameters and integer
 literal type arguments (`Foo(N: int)`, `Foo(u8, 1024)`) ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2016`).
...
- Defined in the native backend subset: 128-bit scalar primitives
 (`i128`, `u128`, `f128`).
 - In the current scalar-slot model ([`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)),
...
- Regexp: `regexp`
 ...
 compilation and matching helpers live in [`std::regex`](/silk/docs/std/regex/) (see [`std::regex`](/silk/docs/std/regex/)).
...
- Arrays / Slices: `T[]`, `T[N]`
 ...
 [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/) for the current scalar-slot memory
...
- `type` aliases may be exported (`export type ...;`) and imported as type names
 via named file imports (see [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/)).
...
- Function values are supported end-to-end for this subset (non-capturing and
 ...
 in [`Memory Model (Stack, Heap, and Moves)`](/silk/docs/language/memory-model/).
...
- [`Numeric Literals`](/silk/docs/language/literals-numeric/)
- [`Duration Literals`](/silk/docs/language/literals-duration/)
- [`Boolean Literals`](/silk/docs/language/literals-boolean/)
- [`Character Literals`](/silk/docs/language/literals-character/)
- [`String Literals`](/silk/docs/language/literals-string/)
- [`Regular Expression Literals`](/silk/docs/language/literals-regexp/)
- [`Aggregate Literals`](/silk/docs/language/literals-aggregate/)
...
- Duration literal tokens of the form `<number><unit>` (no whitespace) such as
 `500ms` and `1.5s` (specified in [`Duration Literals`](/silk/docs/language/literals-duration/)).
...
- [`Duration Literals`](/silk/docs/language/literals-duration/) (duration literals like `5ms`, `1.5s`)
...
- [`Character Literals`](/silk/docs/language/literals-character/) (shared escape spellings)
- `C99 ABI and `libsilk.a`` (C ABI string representation)

## Regular Expression Literals

Regular expression literals represent `regexp` values: compiled regular
expression bytecode that can be used by [`std::regex`](/silk/docs/std/regex/) helpers.
...
- `g` — global (recorded; does not change [`std::regex::matches`](/silk/docs/std/regex/) semantics)
- `i` — ignore case
- `m` — multiline
- `s` — dotAll
- `y` — sticky
- `d` — indices (recorded; not surfaced by [`std::regex`](/silk/docs/std/regex/) helpers)
...
- The bytecode format is owned by the runtime regex engine; `regexp` values are
 opaque and must be consumed via [`std::regex`](/silk/docs/std/regex/).
- A literal `regexp` is borrowed data, not a heap-owned regex object:
 [`std::regex::RegExp.compile(...)`](/silk/docs/std/regex/) is the owning/runtime-allocated path, while
 wrapping a literal in [`std::regex::RegExp`](/silk/docs/std/regex/) does not transfer ownership.
- When a foreign ABI caller supplies a malformed `regexp` buffer to
 [`std::regex`](/silk/docs/std/regex/), the runtime rejects it as invalid input before entering the
 bundled engine.
...
- Literal compilation uses the same conservative regexp compile stack budget as
 runtime [`std::regex::RegExp.compile(...)`](/silk/docs/std/regex/); excessively deep patterns are
...
- `Types` (`regexp`)
- [`Formal Grammar Spec`](/silk/docs/language/grammar/) (regexp literal grammar)
- [`std::regex`](/silk/docs/std/regex/) (runtime regex API)
...
If you need multiple characters, use `string` literals ([`String Literals`](/silk/docs/language/literals-string/)).
...
- **Confusing `\xNN` between `char` and `string`**:
 ...
 - for `string`, `\xNN` denotes a raw byte ([`String Literals`](/silk/docs/language/literals-string/)).

### Related Documents

- `Types` (primitive `char` and `string`)
- [`String Literals`](/silk/docs/language/literals-string/) (string literals and escape sequences)
...
- Enforce current-subset restrictions on which element types are supported for
 ...
 [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)).
...
- Respect struct lowering/layout rules from [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/).

#### Notes

The current compiler implementation supports struct literals only for the
limited struct subset described in [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/):
...
- ``while` Loop` ([`while`](/silk/wiki/language/flow-while/) statement semantics)
...
- [`loop`](/silk/wiki/language/flow-loop/) loops (``loop` Loop`)
- [`while`](/silk/wiki/language/flow-while/) loops (``while` Loop`)
...
- `return` statements, including “all paths must return” checking for non-`void`
 functions (``return``)
- [`match`](/silk/wiki/language/flow-match/) as an expression for optionals and enums (``match` Expression (and Statement)`)
- [`match`](/silk/wiki/language/flow-match/) as a statement for typed errors (`Typed Errors (`error`, `panic`, and `T | ErrorType...`)`)
- Expression statements for calls and assignments only
 ([`Expression Statements`](/silk/docs/language/flow-expression-statements/))
...
- [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/) (error codes)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) (working examples)
...
- ``break``
- ``continue``
- [`Blocks and Statement Composition`](/silk/docs/language/flow-blocks-statements/)
- [`Expression Statements`](/silk/docs/language/flow-expression-statements/)
...
- The condition must have type `bool`. If it does not, the checker reports a
 type mismatch ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2001`).
...
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
...
[`while`](/silk/wiki/language/flow-while/). This is part of Formal Silk (see `Formal Silk`).
...
- A `break` outside a loop is a type-check error ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/),
 `E2007`).
...
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) (shows `break` in a loop with specs)
...
- transfers control to the loop’s “next iteration” point:
 - for [`loop`](/silk/wiki/language/flow-loop/), this means jumping to the start of the loop body.
 - for [`while`](/silk/wiki/language/flow-while/), this means re-evaluating the loop condition.
...
- A `continue` outside a loop is a type-check error ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/),
 `E2008`).
...
- `continue;` is accepted inside loops ([`loop`](/silk/wiki/language/flow-loop/), [`while`](/silk/wiki/language/flow-while/), and `for`) and lowered
 end-to-end.
...
- In a function with non-`void` result, falling off the end of the function
 body is a compile-time error ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2010`).
...
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) (wrong type, rejected)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) (missing return, rejected)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/) (uses `return;` in a `-> void` method)
...
- Stack vs heap allocation is specified in [`Memory Model (Stack, Heap, and Moves)`](/silk/docs/language/memory-model/).
...
- The method named `constructor` is treated specially:
 ...
 default constructor exists (see [`Aggregate Literals`](/silk/docs/language/literals-aggregate/)),
...
- This supports fluent chaining for value-consuming helper APIs like
 [`std::result::Result.unwrap_or`](/silk/docs/std/result/):
...
See [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/) for module-set rules and for how
...
- ``match` Expression (and Statement)` (match expression rules)
- [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/) (struct payloads)
- `Types` (nominal types and type annotations)
- [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/) (namespaces and imports)
...
small implicit std prelude (see [`Packages, Imports, and Exports`](/silk/docs/language/packages-imports-exports/)).
In particular, the interface names from [`std::interfaces`](/silk/docs/std/interfaces/) are available without
an explicit `import std::interfaces;`. This prelude is specified by the stdlib
module [`std::runtime::globals`](/silk/docs/std/runtime-globals/).
...
- Exception (static protocol, Supported forms):
 - [`std::interfaces::Deserialize(S)`](/silk/docs/std/interfaces/) and [`std::interfaces::Parse(E, S)`](/silk/docs/std/interfaces/) are
...
value is rewritten by the checker into an ordinary [`match`](/silk/wiki/language/flow-match/) dispatch over that
union.

Typed-binder [`match`](/silk/wiki/language/flow-match/) patterns may also name an interface. When the scrutinee has
...
- typed-binder [`match`](/silk/wiki/language/flow-match/) arms whose pattern type is an implemented interface for
 a concrete struct scrutinee,
- special-case compiler hooks for specific interfaces (currently
 [`std::interfaces::Drop`](/silk/docs/std/interfaces/) for deterministic cleanup; see
 [`std::interfaces`](/silk/docs/std/interfaces/) and [`Memory Model (Stack, Heap, and Moves)`](/silk/docs/language/memory-model/)).
...
- and lowering interface method calls to ordinary [`match`](/silk/wiki/language/flow-match/) dispatch over that
 union.
...
- Package imports: [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/), [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/), [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- File imports (named + default): [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/),
 [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/),
 [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)
- Package-path imports (`from pkg::name`): [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/),
 [`examples/`](https://github.com/oro-computer/silk/tree/master/examples/)

When an import fails, the relevant error codes live in [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/)
...
- The `export` modifier marks a declaration as part of the package’s
 ...
 [`Compiler Architecture`](/silk/docs/compiler/architecture/).
...
- a multi-module helper (`the module-set import helper`) seeds each
...
The [`Optional`](/silk/docs/language/optional/) type provides a safe way to represent values that may or may not
...
- Use [`match`](/silk/wiki/language/flow-match/), `?.` (optional chaining), and `??` (coalescing) to consume optionals.
...
- The type system (`the implementation`) models optional types, and the parser
 ...
 [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2015`):
...
- The current `linux/x86_64` IR→ELF backend subset now supports a first slice
 ...
 - explicitly branch on optionals via the [`match`](/silk/wiki/language/flow-match/) expression (see
 ``match` Expression (and Statement)`),
 ...
 including nested structs and optionals; see [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)).
...
- Not yet implemented:
 ...
 - [`match`](/silk/wiki/language/flow-match/) over non-optional scrutinee types (and richer pattern forms beyond
 `None`/`Some(...)`),
...
- The optional and recoverable-result forms are distinguished by the left-hand
 operand type; expression [`match`](/silk/wiki/language/flow-match/) remains the more general payload-aware tool
...
- Explicit checking via [`match`](/silk/wiki/language/flow-match/):
 - Pattern‑matching on `Some(...)` / `None` to handle both cases explicitly.

### Optional combinators (methods)

In addition to [`match`](/silk/wiki/language/flow-match/), `?.`, and `??`, the compiler provides a small set of
...
- Support [`match`](/silk/wiki/language/flow-match/) on `Option(T)` and integrate optionals with flow control and error reporting.
...
- **Recoverable errors** (invalid user input, I/O failures, parse failures): model
 these as normal values, typically using [`std::result::Result(T, E)`](/silk/docs/std/result/) or an
 optional (`T?`).
...
[`std::url`](/silk/docs/std/url/) exposes a recoverable parsing API ([`std::url::parse`](/silk/docs/std/url/)) that returns a
tagged result ([`std::url::URLResult`](/silk/docs/std/url/)), so callers can report an error and keep
going without aborting.

A runnable example that wraps `URLResult` into [`std::result::Result`](/silk/docs/std/result/) and parses
...
- `if` / [`match`](/silk/wiki/language/flow-match/) can be used to branch on error vs. success cases.
...
- [`std::result`](/silk/docs/std/result/) (recoverable `Result(T, E)`),
- [`std::url`](/silk/docs/std/url/) and `an example program`
...
- The [`match`](/silk/wiki/language/flow-match/) expression also supports `Ok(...)` / `Err(...)` patterns for
 `Result` values (see ``match` Expression (and Statement)`).
...
[`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/)).
...
[`Regions`](/silk/docs/language/regions/), [`Borrow Checking (Static Alias and Lifetime Safety)`](/silk/docs/language/borrow-checker/), and `the implementation status`
for current scope.
...
FFI boundary (see [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/) and
`External Declarations (`ext`)`).
...
atomic/synchronized handles ([`std::sync`](/silk/docs/std/sync/) or [`std::atomic`](/silk/docs/std/atomic/)), pass non-owning
`*Borrow` handle views across tasks, or use [`std::sync::Arc(T)`](/silk/docs/std/sync/) when task-safe
shared ownership is required.

Atomic operations do not change the `new` reference model. [`std::atomic`](/silk/docs/std/atomic/)
...
[`std::sync::Arc(T)`](/silk/docs/std/sync/) is a separate shared-ownership tool. Its retain/release
...
region context is established with `with` (see [`Regions`](/silk/docs/language/regions/)).
...
resource-owning `struct` values via [`std::interfaces::Drop`](/silk/docs/std/interfaces/).
...
- Values that require deterministic cleanup should be treated as
 ...
 [`Borrow Checking (Static Alias and Lifetime Safety)`](/silk/docs/language/borrow-checker/)).
...
proposed surface form must be written down in [`Formal Grammar Spec`](/silk/docs/language/grammar/) before
it is implemented.
...
- See [`Mutability`](/silk/docs/language/mutability/) for the current implemented borrow rules
 (call-scoped aliasing checks for `&T` parameters in the Supported forms).
- See [`Borrow Checking (Static Alias and Lifetime Safety)`](/silk/docs/language/borrow-checker/) for the broader planned borrow checker.
...
- Refutable-pattern binders also preserve borrow identity when the scrutinee
 ...
 including monomorphized [`std::result::Result(T, E)`](/silk/docs/std/result/) instantiations, and the
...
- [`match`](/silk/wiki/language/flow-match/) expressions preserve borrow identity under the same conservative
 rule:
...
Any such extension must be specified in [`Formal Grammar Spec`](/silk/docs/language/grammar/) and in this
document before implementation lands, and must be reflected in diagnostics
([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/)) and tests.
...
For ordinary application code, prefer [`std::sync`](/silk/docs/std/sync/) primitives such as mutexes,
...
The public ordering enum is [`std::atomic::Ordering`](/silk/docs/std/atomic/):
...
- [`std::atomic::Ordering`](/silk/docs/std/atomic/),
- [`std::atomic::AtomicBool`](/silk/docs/std/atomic/),
- [`std::atomic::AtomicBoolBorrow`](/silk/docs/std/atomic/),
- [`std::atomic::AtomicU64`](/silk/docs/std/atomic/),
- [`std::atomic::AtomicU64Borrow`](/silk/docs/std/atomic/),
- [`std::atomic::fence`](/silk/docs/std/atomic/).
...
- Hosted async runtime bring-up exists on supported hosted POSIX targets
 ...
 [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) surface (see [`Async Runtime (Hosted)`](/silk/docs/compiler/async-runtime/)).
...
[`Async Runtime (Hosted)`](/silk/docs/compiler/async-runtime/); the runtime backlog items for the current
...
headers ([`vendor/include`](https://github.com/oro-computer/silk/tree/master/vendor/include)) directly into the compiler when the host archive is
present:

- `linux/x86_64` -> [`vendor/lib/x64-linux/libz3.a`](https://github.com/oro-computer/silk/blob/master/vendor/lib/x64-linux/libz3.a)
- `macos/aarch64` -> [`vendor/lib/aarch64-macos/libz3.a`](https://github.com/oro-computer/silk/blob/master/vendor/lib/aarch64-macos/libz3.a) (optional and staged
 when present)
...
- and const-evaluable module/package `const` bindings, including exported
 qualified names such as [`std::limits::I64_MAX`](/silk/docs/std/limits/).
...
- Using a `#const` name in a runtime expression (e.g. in [`while`](/silk/wiki/language/flow-while/) conditions or normal
...
For [`while`](/silk/wiki/language/flow-while/) loops, the initial surface syntax is:
...
- One or more `#invariant` annotations, zero or more `#monovariant` annotations,
 ...
 [`while`](/silk/wiki/language/flow-while/) keyword.
- Each annotation is terminated by a semicolon.
- The compiler front-end:
...
- `#invariant` / `#variant` / `#monovariant` on [`while`](/silk/wiki/language/flow-while/) loops:
...
- Verification of the full expression language and full statement language
 ([`match`](/silk/wiki/language/flow-match/), nested loops, indirect calls, and many operators are not supported
...
`T?` in the Supported forms (see [`Optional`](/silk/docs/language/optional/)). This is still
...
type checking ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2016`). See
[`Generics (Monomorphized)`](/silk/docs/language/generics/).
...
([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2016`).
...
future work. See [`Concurrency`](/silk/docs/language/concurrency/) for the concurrency model and
...
These APIs are not yet present in the in-tree [`std/`](https://github.com/oro-computer/silk/tree/master/std/) implementation.
...
- Lowering/codegen implements `task` execution using OS threads on `linux/x86_64`
 ...
 (`the implementation`) so `await` is a true suspension point:
 ...
 [`std::runtime::event_loop`](/silk/docs/std/runtime-event_loop/) API; see [`Async Runtime (Hosted)`](/silk/docs/compiler/async-runtime/).
...
- Function expressions are implemented as first-class function values:
 ...
 scalar `&T` parameters (for example `&int`) are supported in the Supported forms.
...
introduced by the literal syntax itself (see [`String Literals`](/silk/docs/language/literals-string/)), not by the
`const fn` evaluator.
...
arguments (“varargs”). This is used heavily by [`std::io::print`](/silk/docs/std/io/) /
[`std::io::println`](/silk/docs/std/io/) for formatted output.
...
In the compiler, a varargs parameter is lowered as a
...
from [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/) when one exists.
...
- [`Formal Grammar Spec`](/silk/docs/language/grammar/) for the exact accepted syntax,
- `Types` for type-system rules and special cases,
- [`Mutability`](/silk/docs/language/mutability/) for `mut` and borrowing rules,
- [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/) for error codes,
...
- Comparison: `==`, `!=`, `<`, `<=`, `>`, `>=`.
 ...
 - In the backend, `==` and `!=` are also defined for `bool`
 operands.
 - In the backend, comparisons are also defined for `Duration`
 and `Instant` when both operands have the same time type.
 - In the backend, `==` and `!=` are also defined for `string`
 operands, comparing the underlying UTF-8 byte sequences for equality
 (length check + bytewise compare).
 - In the backend, ordered comparisons over `string`
 ...
 - In the backend, `==` and `!=` are also defined for supported
 ...
 - In the backend, `==` and `!=` are also defined for the
 supported `struct` subset (see [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)),
...
- Ranges / varargs delimiters: `..`, `..=`, `...`.
 ...
 - `...` is the varargs/rest marker (see [`Varargs (Variable Arguments)`](/silk/docs/language/varargs/)).
- Other punctuation: `?`, `??`, `->`, `=>`, `,`, `;`, `(`, `)`, `{`, `}`, `[`, `]`, `_`, `:`.
 - Currently, `??` is supported for:
 - optionals in the backend (including scalar, `string`, and
 ...
 ([`std::result`](/silk/docs/std/result/)), and
...
- The left-hand side must be an assignable lvalue. In the Supported forms, it may be:
 ...
 In the backend, nested field assignment is supported only
...
- Sizes reflect the current scalar-slot lowering model ([`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)):
...
- Alignments reflect the current scalar-slot lowering model
 ([`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)): values are stored as 8-byte slots.
...
- Offsets reflect the current scalar-slot lowering model
 ([`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)): each lowered slot is stored in an
...
- `u64`/`usize` ↔ `T[]` / `T[N]` (unsafe pointer/slice view cast):
 ...
 - In the scalar-slot backend, indexed accesses through
...
- and FFI-safe conversions and APIs in [`std::temporal`](/silk/docs/std/temporal/).

Compiler requirements:

- Implement type-checking and lowering for the operator subset above.
- Implement duration literal parsing as specified in [`Duration Literals`](/silk/docs/language/literals-duration/).
- Integrate with [`std::temporal`](/silk/docs/std/temporal/) in the standard library.
...
- A raw dynamic symbol address can be converted explicitly with
 ...
 loader APIs such as [`std::dylib`](/silk/docs/std/dylib/); Silk does not validate the symbol signature
...
- Runtime regex helpers also track which bytecode buffers they actually
 allocated: only [`std::regex::RegExp.compile(...)`](/silk/docs/std/regex/) produces an owned regex
...
- describe how user‑defined structs map to C structs (respecting the layout rules in [`Structs, Impl Blocks, and Memory Layout`](/silk/docs/language/structs-impls-layout/)),
...
The full language design includes rich optional patterns (`?.`, [`match`](/silk/wiki/language/flow-match/), nested
...
- `asm` is currently implemented only for the native `linux/x86_64` and
 `macos/aarch64` backends.
...
- `asm` is target-dependent by nature. The current implementation is supported
 only for the native `linux/x86_64` and `macos/aarch64` backends.
...
- any `asm` support on non-`linux/x86_64` targets/backends.
 except for the current `macos/aarch64` host Mach-O subset.

## Blocks and Statement Composition

Blocks group statements, establish lexical scopes, and provide the “body” form
for structured control-flow constructs like `if`, [`while`](/silk/wiki/language/flow-while/), and the [`match`](/silk/wiki/language/flow-match/)
...
[`Formal Grammar Spec`](/silk/docs/language/grammar/) for exact syntax):
...
- Structured blocks: `async { ... }` / `task { ... }` (see [`Concurrency`](/silk/docs/language/concurrency/)).
- Expression statements: limited to calls, assignments, and increment/decrement
 in the Supported forms ([`Expression Statements`](/silk/docs/language/flow-expression-statements/)).
- Flow control:
 ...
 - `assert`,
 - `panic` (typed errors),
 - [`match`](/silk/wiki/language/flow-match/) statement (typed errors; see `Typed Errors (`error`, `panic`, and `T | ErrorType...`)`).
...
[`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2014`).

#### Blocks as Expressions

The broader language design includes expression-oriented flow constructs (for
example [`match`](/silk/wiki/language/flow-match/) expressions today and `if` expressions).
...
- the toolchain metadata constants listed below are available as
 built-in compile-time constants in every module.
- `silk --version` reports the same toolchain version, ABI version,
 and git commit.
...
- the `SILK_ABI_VERSION_*` macros in [`include/silk/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk/silk.h), and
...
- CLI output: the `silk` CLI and [`silk(1)`](/silk/docs/man/silk.1/) (`silk --version`)
...
- build metadata is available to runtime code via [`std::runtime::build`](/silk/docs/std/runtime-build/)
 ([`std::runtime`](/silk/docs/std/runtime/)).
- build metadata is available as built-in compile-time constants:
...
- Debug stack traces and debug assertion behavior are controlled separately by
 [`std::runtime::build::is_debug()`](/silk/docs/std/runtime-build/) (see [`std::runtime`](/silk/docs/std/runtime/)).
...
### Relationship to [`std::runtime::build`](/silk/docs/std/runtime-build/)

The standard library provides [`std::runtime::build`](/silk/docs/std/runtime-build/) functions that return the
same metadata:

- [`std::runtime::build::kind() -> string`](/silk/docs/std/runtime-build/)
- [`std::runtime::build::mode() -> string`](/silk/docs/std/runtime-build/)
- [`std::runtime::build::version() -> string`](/silk/docs/std/runtime-build/)

Use [`std::runtime::build`](/silk/docs/std/runtime-build/) when you prefer explicit namespacing or when writing
code intended to run under alternate stdlib roots.

The same module also owns the reusable Formal Silk vocabulary for build
metadata:

- [`std::runtime::build::build_kind_is(...)`](/silk/docs/std/runtime-build/)
- [`std::runtime::build::build_mode_is(...)`](/silk/docs/std/runtime-build/)
- [`std::runtime::build::{requires_debug_mode,requires_release_mode,requires_executable_kind,requires_object_kind,requires_static_kind,requires_shared_kind}`](/silk/docs/std/runtime-build/)
- [`std::runtime::build::build_version_at_least(...)`](/silk/docs/std/runtime-build/)
...
For semver parsing and comparison, use [`std::semver`](/silk/docs/std/semver/) at runtime.
...
[`match`](/silk/wiki/language/flow-match/) (see `the standard library`; targets not covered by the current enum set map
to `Unknown`).
...
See [`std::test`](/silk/docs/std/test/) for the detailed API.
...
value (for example [`match`](/silk/wiki/language/flow-match/) expressions; see ``match` Expression (and Statement)`).
...
- `<step>` is a statement-like expression (the same restricted subset as
 expression statements; see [`Expression Statements`](/silk/docs/language/flow-expression-statements/)).
...
instance method (typically by implementing [`std::interfaces::Iterator(T)`](/silk/docs/std/interfaces/)).
...
array/slice iteration. To write other loops today, use [`while`](/silk/wiki/language/flow-while/):
...
- In Silk currently, `async loop` / `task loop` follow the same
 ...
 ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2031`).
...
- `break;` outside a loop is rejected ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2007`).
- `continue;` outside a loop is rejected ([`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/), `E2008`).
...
- ``match` Expression (and Statement)` ([`match`](/silk/wiki/language/flow-match/) over union values).
...
- **Nominal POD structs** (including `error` types) and **nominal POD enums**
 that lower to a scalar-slot representation in the backend (no
 opaque structs).
...
### Matching ([`match`](/silk/wiki/language/flow-match/))

Union values are consumed via [`match`](/silk/wiki/language/flow-match/) expressions using **typed binder**
patterns:
...
documentation. [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/) remains the normative prose
...
- Primary header: [`include/silk/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk/silk.h).
- Legacy compatibility shim: [`include/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk.h).

Embedders should prefer `#include <silk/silk.h>`. The flat [`include/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk.h)
wrapper remains available for compatibility during the transition.

#### Linking When Static Z3 Is Bundled

When the host-native [`vendor/lib/<host-layout>/libz3.a`](https://github.com/oro-computer/silk/blob/master/vendor/lib/<host-layout>/libz3.a) archive is present,
...
#### Initial C Header Shape ([`include/silk/silk.h`](https://github.com/oro-computer/silk/blob/master/include/silk/silk.h))
...
- Configuration:
 ...
 - `linux-x86_64` (default), and common `x86_64-*-linux-gnu` triples such as
 `x86_64-linux-gnu` and `x86_64-unknown-linux-gnu`,
 - `linux-x86_64-musl`, and common `x86_64-*-linux-musl` triples such as
 ...
 `main()` (for example via [`std::args::{argc,argv,current}`](/silk/docs/std/args/)),
 ...
 need to manually add libc when using hosted `std::` modules like [`std::io`](/silk/docs/std/io/)
 and [`std::fs`](/silk/docs/std/fs/). Additional non-libc dependencies must still be declared via
...
- Building artifacts:
 ...
 static archive [`vendor/lib/<host-layout>/libz3.a`](https://github.com/oro-computer/silk/blob/master/vendor/lib/<host-layout>/libz3.a),
 ...
 - `build/lib/silk/std/libsilk_std.a` when using the in-repo [`std/`](https://github.com/oro-computer/silk/tree/master/std/) root, otherwise
 ...
 - regex literals and other borrowed `regexp` views are not caller-owned heap objects; only [`std::regex::RegExp.compile(...)`](/silk/docs/std/regex/) produces runtime-owned regex bytecode,
 - when [`std::regex`](/silk/docs/std/regex/) executes a foreign ABI-supplied `regexp`, the bundled runtime first validates the bytecode header/control-flow shape and reports malformed inputs as `EXEC_ERR_INVALID_INPUT` instead of entering the engine blindly,
 ...
 - other string operations (concatenation, indexing, etc.) are not implemented yet; higher-level regex matching lives in [`std::regex`](/silk/docs/std/regex/) and is routed through `ext` calls.
 ...
 **compile‑time boolean literal** (`true` or `false`) and each
 ...
 - one or more **trivial constant [`while`](/silk/wiki/language/flow-while/) loops** that appear before
 ...
 - ELF64 for `linux-x86_64`, `linux-x86_64-musl`,
 `linux-aarch64`, `linux-aarch64-musl`, and `android-aarch64`,
...
- Error reporting:
 ...
 - The text format and initial stable error code set are specified in [`Compiler Diagnostics`](/silk/docs/compiler/diagnostics/). Embedders should treat the formatted message as user-facing text (not a stable machine-readable protocol).
...
- [`libsilk(7)`](/silk/docs/man/libsilk.7/) — C99 ABI manpage for embedders.
