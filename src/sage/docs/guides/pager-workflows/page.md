...
section: "guides"
order: 2
sourcePath: "guides/pager-workflows.md"
...
[`sage`](/sage/docs/cli/sage/) is most useful when you treat it as a CLI tool first and a programmable runtime second. These patterns cover the common downstream workflows.
...
When stdin is piped and stdout is still a TTY, [`sage`](/sage/docs/cli/sage/) keeps the interactive TUI and reads keys from `/dev/tty`.
...
If you need a narrower set, hand the file list to [`sage`](/sage/docs/cli/sage/) explicitly:
...
When `stdout` is not a TTY, [`sage`](/sage/docs/cli/sage/) does not enter the TUI. It falls back to a streaming branch.
...
See [Inputs and modes](/sage/docs/cli/inputs-and-modes/) for the exact rules.
