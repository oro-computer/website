...
section: "cli"
order: 5
sourcePath: "cli/inputs-and-modes.md"
...
This page is the exact downstream input model for [`sage`](/sage/docs/cli/sage/).
...
- [`sage`](/sage/docs/cli/sage/) with no paths and interactive stdin prints help instead of blocking
...
If `stdout` is a TTY, [`sage`](/sage/docs/cli/sage/) starts the TUI.
...
When stdin is piped and stdout is still a TTY, [`sage`](/sage/docs/cli/sage/) reads keys from `/dev/tty` so the session stays interactive.

### Pass-through mode

If `stdout` is not a TTY, [`sage`](/sage/docs/cli/sage/) does not enter the TUI.
...
For URLs, [`sage`](/sage/docs/cli/sage/) ignores query strings and fragments for syntax selection.
...
If the URL path has no useful extension, [`sage`](/sage/docs/cli/sage/) may use the HTTP `Content-Type` as a hint. The built-in content-type mapping includes:
