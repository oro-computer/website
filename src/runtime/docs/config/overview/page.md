...
At build/run time, [`oroc`](/runtime/docs/cli/oroc/) computes an effective configuration from multiple sources:
...
[`oroc`](/runtime/docs/cli/oroc/) will read `.ororc` from several locations (system, user, and project). The project-local `.ororc` has the
highest precedence.
...
- Reference: [Config keys](/runtime/docs/config/reference/)
- Bundling inputs: [copy_map](/runtime/docs/config/copy-map/)
- CLI: [`oroc config`](/runtime/docs/cli/config/)
