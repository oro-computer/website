...
section: "cli"
order: 3
sourcePath: "cli/slg.md"
githubRepo: "oro-computer/slg"
githubRef: "master"
---

# [`slg`](/slg/docs/cli/slg/)

[`slg`](/slg/docs/cli/slg/) is the canonical CLI entrypoint for Silk Line Grep.
...
[`slg`](/slg/docs/cli/slg/) is **filesystem-only**:
...
See [Input types and parsing](/slg/docs/cli/input-types-and-parsing/) for the exact rules.

## Matching defaults

- regex is the default public surface
- patterns with no regex metacharacters are automatically handled by the fixed matcher currently
- `-F`, `-Q`, `--fixed-string`, `--literal`, and `--fixed-strings` force literal matching
- `-i` enables case-insensitive matching
- `-S` enables smart-case behavior

See [Patterns and matching](/slg/docs/cli/patterns-and-matching/).

## Traversal defaults

By default [`slg`](/slg/docs/cli/slg/):
...
See [Traversal and ignores](/slg/docs/cli/traversal-and-ignores/).
...
See [Output and exit codes](/slg/docs/cli/output-and-exit-codes/) and [Diagnostics](/slg/docs/cli/diagnostics/).

## No config file

[`slg`](/slg/docs/cli/slg/) does not currently read a user config file or project config file at runtime.

The only runtime environment knob is `NO_COLOR`, and only when color mode is `auto`.

## Next

- Exact roots and positionals: [Input types and parsing](/slg/docs/cli/input-types-and-parsing/)
- Exhaustive flags: [Options reference](/slg/docs/cli/options-reference/)
- Reader-friendly manual: [`slg(1)`](/slg/docs/man/slg-1/)
