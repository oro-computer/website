...
section: "cli"
order: 4
sourcePath: "cli/input-types-and-parsing.md"
...
This page is the exact downstream input model for [`slg`](/slg/docs/cli/slg/).
...
- if no root paths are provided, [`slg`](/slg/docs/cli/slg/) uses `.`
...
If a root is a file, [`slg`](/slg/docs/cli/slg/) searches or lists that file directly.
...
If a root is a directory, [`slg`](/slg/docs/cli/slg/) walks it recursively.
...
[`slg`](/slg/docs/cli/slg/) does **not** currently have:
...
The first form pipes bytes to [`slg`](/slg/docs/cli/slg/), but [`slg`](/slg/docs/cli/slg/) has no stdin search path and will still parse only filesystem roots. The second form treats `-` as a literal root path named `-`.
...
[`slg`](/slg/docs/cli/slg/) does not load a config file or environment-based runtime profile. Use CLI flags for behavior changes.
