...
export function blogIndex(
  ...
  archives: readonly Pick<BlogArchive, 'year' | 'url'>[],
  year?: string,
): string {
  return render(html`
      ...
      ${archives.length
        ? html`
            <nav class="blog-years" aria-label="Blog archives">
              <a href="/blog/" ${!year ? html`aria-current="page"` : null}>
                All posts
              </a>
              ${archives.map(archive => html`
                <a
                  href="${archive.url}"
                  ${archive.year === year ? html`aria-current="page"` : null}
                >
                  ${archive.year}
                </a>
              `)}
    ...
    ${posts.length
      ...
      : html`
          <p class="blog-empty">
            No posts yet. Subscribe to a feed to follow future posts.
          </p>
        `}
  `)
}
