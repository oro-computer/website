...
- `/pattern/flags` produces a `regexp` value, compiled at compile time (see
 [literals regexp](?p=language/literals-regexp)).
...
- [literals regexp](?p=language/literals-regexp) (regex literals)
- [types](?p=language/types) (`regexp`)
- [unicode](?p=std/unicode) (Unicode helpers used by the regex runtime)
- [number](?p=std/number) (numeric parsing/formatting)
