...
Source: `std/runtime/sync.slk`

This is the exact canonical documentation page for `std::runtime::sync`.

## Role

`std::runtime::sync` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/sync.slk`.
- The canonical module name is `std::runtime::sync`.
