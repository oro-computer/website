...
([structs impls layout](?p=language/structs-impls-layout)). At ABI boundaries (`export fn` and
...
`size_t`. This is not ABI-safe to call directly from Silk in the Supported forms.
...
- static archives into `vendor/lib/<host-layout>/` (for linking),

alongside existing vendored deps. See [vendored deps](?p=compiler/vendored-deps).
