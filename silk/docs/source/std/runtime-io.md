...
Source: `std/runtime/io.slk`

This is the exact canonical documentation page for `std::runtime::io`.

## Role

`std::runtime::io` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/io.slk`.
- The canonical module name is `std::runtime::io`.
