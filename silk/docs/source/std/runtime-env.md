...
Source: `std/runtime/env.slk`

This is the exact canonical documentation page for `std::runtime::env`.

## Role

`std::runtime::env` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/env.slk`.
- The canonical module name is `std::runtime::env`.
