...
Source: `std/runtime/number.slk`

This is the exact canonical documentation page for `std::runtime::number`.

## Role

`std::runtime::number` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/number.slk`.
- The canonical module name is `std::runtime::number`.
