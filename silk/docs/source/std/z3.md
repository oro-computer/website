...
`silk build` auto-links the vendored `libz3.a` into executable and
shared-library outputs. Auto-linking also applies on that target when linking
`.o`/`.a` inputs that reference Z3 `Z3_*` symbols.

Musl targets do not use a vendored Z3 archive. A Linux x86_64 musl build that
...
The current toolchain provides vendored `std::runtime::z3` auto-linking only on
Linux x86_64 glibc.
