# `std::runtime::wasi::sync`

Source: `std/runtime/wasi/sync.slk`

This is the exact canonical documentation page for `std::runtime::wasi::sync`.

## Role

`std::runtime::wasi::sync` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/wasi/sync.slk`.
- The canonical module name is `std::runtime::wasi::sync`.
