# `std::runtime::posix::mem`

Source: `std/runtime/posix/mem.slk`

This is the exact canonical documentation page for `std::runtime::posix::mem`.

## Role

`std::runtime::posix::mem` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/posix/mem.slk`.
- The canonical module name is `std::runtime::posix::mem`.
