...
- `std::runtime::mem` — low-level allocation and compiler-backed intrinsics used by
 ...
 - when an active region context is established with `with` ([regions](?p=language/regions)),
...
- `std::runtime::io` — low-level stdio primitives used by `std::io` (on
 ...
 It also includes PTY helpers used by `std::process::child::Command.spawn_pty`
...
- `std::runtime::regex` / `std::runtime::unicode` / `std::runtime::number` / `std::runtime::readline` —
 ...
 - `std::runtime::readline` now also includes process-global completion-list
 management used by the public `std::readline::{clear_completions,add_completion}`
 surface.
 ...
 `include/silk/rt.h`) before invoking any `silk_rt_*` entrypoints. This hook
...
- Async event loop / executor integration (`std::runtime::event_loop`) for hosted `async`/`await`:
 ...
 (see [async runtime](?p=compiler/async-runtime)).
...
[howto custom stdlib root](?p=usage/howto-custom-stdlib-root).
