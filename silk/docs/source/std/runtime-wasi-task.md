# `std::runtime::wasi::task`

Source: `std/runtime/wasi/task.slk`

This is the exact canonical documentation page for `std::runtime::wasi::task`.

## Role

`std::runtime::wasi::task` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/wasi/task.slk`.
- The canonical module name is `std::runtime::wasi::task`.
