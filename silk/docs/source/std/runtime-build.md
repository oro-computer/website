...
Source: `std/runtime/build.slk`

This is the exact canonical documentation page for `std::runtime::build`.

## Role

`std::runtime::build` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/build.slk`.
- The canonical module name is `std::runtime::build`.
