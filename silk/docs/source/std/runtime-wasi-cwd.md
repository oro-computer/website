# `std::runtime::wasi::cwd`

Source: `std/runtime/wasi/cwd.slk`

This is the exact canonical documentation page for `std::runtime::wasi::cwd`.

## Role

`std::runtime::wasi::cwd` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/wasi/cwd.slk`.
- The canonical module name is `std::runtime::wasi::cwd`.
