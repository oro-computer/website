...
(`/** ... */` or `/// ...`) per [doc comments](?p=language/doc-comments).
...
- The built-in `string` type is a UTF-8 byte sequence represented as a
 `{ ptr, len }` pair (see [literals string](?p=language/literals-string) and
 [ext](?p=language/ext)).
...
- Contracts must reflect real runtime invariants (avoid over-strong
 ...
 on preconditions.
- See [formal verification](?p=language/formal-verification) for the Formal Silk model. Note
