...
Source: `std/runtime/task.slk`

This is the exact canonical documentation page for `std::runtime::task`.

## Role

`std::runtime::task` is an implementation-facing runtime module in the shipped `std/**` tree.
...
- The shipped source for this module is `std/runtime/task.slk`.
- The canonical module name is `std::runtime::task`.
