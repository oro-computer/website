...
Exact alias matches are preferred before free-text fallback:
...
- natural-language guide queries now go through a deterministic intent-routing
 pass before alias/FTS fallback, so task phrasing like `how to read a file`
 or `how can i open a file and read it` resolves to curated file I/O guides,
 while `read from stdin` resolves to the stdin echo guide
