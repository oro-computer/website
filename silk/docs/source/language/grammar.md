...
- Statements:
 ...
 matching and typed errors as specified in [flow match](?p=language/flow-match)
 and [typed errors](?p=language/typed-errors).

 The statement form accepts:

 - the same ordinary-value scrutinee + pattern subsets documented in
 [flow match](?p=language/flow-match), and
