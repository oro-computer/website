...
This document specifies Silk’s function discipline system: how functions
...
Silk implements `pure fn` parsing and a strict purity checker. Concurrency
disciplines (`task` / `async`) are parsed, and `Task(T)` / `Promise(T)` handles
plus `yield` and `await` are documented in `docs/language/concurrency.md`.
...
The checker enforces or targets the following rules:
...
The standard library can provide typed adapters to cross boundaries safely:
...
These APIs are not yet part of the in-tree `std/` implementation.
