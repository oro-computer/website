...
[const functions](?p=language/const-functions). The `const` modifier is orthogonal to the
...
future work. See [concurrency](?p=language/concurrency) for the concurrency model and
...
- Lowering/codegen implements `task` execution using OS threads on `linux/x86_64`
 ...
 `std::runtime::event_loop` API; see [async runtime](?p=compiler/async-runtime).
