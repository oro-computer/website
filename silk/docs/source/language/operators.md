...
- Comparison: `==`, `!=`, `<`, `<=`, `>`, `>=`.
 ...
 supported `struct` subset (see [structs impls layout](?p=language/structs-impls-layout)),
...
- Currently:
 ...
 See [optional](?p=language/optional).
...
- Typed error propagation: `?` (postfix).
 ...
 function; see [typed errors](?p=language/typed-errors).
- Ranges / varargs delimiters: `..`, `..=`, `...`.
 - `..` and `..=` form range literals of type `range` (see [types](?p=language/types)).
 - `...` is the varargs/rest marker (see [varargs](?p=language/varargs)).
- Other punctuation: `?`, `??`, `->`, `=>`, `,`, `;`, `(`, `)`, `{`, `}`, `[`, `]`, `_`, `:`.
...
- Sizes reflect the current scalar-slot lowering model ([structs impls layout](?p=language/structs-impls-layout)):
...
- Alignments reflect the current scalar-slot lowering model
 ([structs impls layout](?p=language/structs-impls-layout)): values are stored as 8-byte slots.
...
- Offsets reflect the current scalar-slot lowering model
 ([structs impls layout](?p=language/structs-impls-layout)): each lowered slot is stored in an
...
- `&Struct` → `&Struct` (safe “shape cast” for references):
 ...
 under different local names. See [mutability](?p=language/mutability).
...
- `as` does not participate in the implicit call-argument coercion mechanism
 described in [types](?p=language/types) (that mechanism is opt-in per
