...
Current subset (requires initializers; see `E2015`):
...
```silk
// Current subset when `main` declares a compatible error set.
...
See `Generics (Monomorphized)` for the current monomorphized subset and the
...
- Implemented current generic surface: parameterized nominal declarations with
  ...
  in type positions (`Wrap(u8, 4)`), and the generic-function surface
  described in `Generics (Monomorphized)`.
...
- Reference (borrow): `&T`
  ...
  - Notes: reference type; in the current subset, `&Struct` may appear in
    parameter types and as local values:
    ...
    Mutability follows the `mut` borrow contract and per-call aliasing rules
    described in `Mutability`.
- Arrays / Slices: `T[]`, `T[N]`
...
- a multi-module package-checking helper seeds each
...
- richer first-class borrow support beyond the current `&Struct` /
...
- structural invariants are captured in `std::formal::slice_well_formed(ptr, len)`
  and `std::formal::vector_well_formed(ptr, len, cap)`,
- bounds checks are expressed via `std::formal::bounds_i64`,
...
- A small hosted stdlib baseline exists now under `std::task`,
  `std::sync`, `std::runtime::event_loop`, and `std::io::async`
  for the hosted `linux/x86_64` subset.
...
#### Function annotations (current syntax)
...
#### Loop annotations (current syntax)
...
This section covers Silk’s dependent-type-like surface: places where a type
...
- const parameters on parameterized declarations (`struct`, `interface`,
  `enum`, `impl`),
- applied types with const arguments (for example `Wrap(u8, 4)`),
- generic functions and methods using the compile-time/runtime split,
- call-site inference of both type and const arguments when runtime arguments
  carry enough type information.

### Const Parameters (Current Subset)

Const parameters are compile-time integer values written with an explicit type
annotation:

```silk
struct VectorN(T, N: usize) {
  buf: T[N],
}
```
...
- `N: usize` is a const parameter whose value is known at compile time.

### Generic Functions (CT/RT Split)
...
The parameters before `;` are compile-time parameters; the parameters after `;`
are ordinary runtime parameters.

Calls may be explicit:
...
What is still beyond the current subset:

- general type-level arithmetic such as `M + N` in type positions,
- solver-backed constraints over const parameters,
- arbitrary value predicates attached to types.

Use `Formal Silk` for value predicates and proof obligations; use const
parameters for compile-time shape.
...
- Configuration:
  ...
  `target_triple` string is copied. The current compiler subset recognizes the
