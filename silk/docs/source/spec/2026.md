...
- Statements (Supported forms):
 ...
 - `ForInStmt ::= 'for' ForBinder 'in' ExprNoRange (RangeOp ExprNoRange)? Block`
...
Dynamic interface dispatch (trait
...
- Package-import aliasing (`import std::runtime::mem;`, `import std::strings as str;`)
 that binds a namespace alias for qualified access (`mem::string_len`, `str::eq`).
...
- Within an inline module body, unqualified name lookup for inline-module
 declarations is not implemented yet in Silk currently; use
...
- If the alias would conflict with an existing in-scope name, the import is a
 ...
 qualified package path (for example `std::runtime::mem::string_len`).
...
Note: in Silk currently, package specifiers are matched literally
...
To share state across tasks, transfer ownership by value, or share via explicit
synchronization (`std::sync`) and pass non-owning `*Borrow` handle views across
tasks.
...
- `Buffer(T)` uses the current compiler’s **scalar-slot** layout (for example
 ...
 `std::buffer::BufferU8`.
- The shipped surface includes operations such as:
...
- `E2001` — type mismatch.
- `E2002` — unsupported construct in the Supported forms (the diagnostic detail
...
- `E2016` — unsupported generic form in the Supported forms (for example const parameters / const type arguments).
...
- `E2068` — capturing closure capture type is not supported in the Supported forms.
- `E2069` — capturing closure may not capture a mutable binding in the Supported forms.
...
- `E2099` — `using` cannot import `constructor` in the Supported forms.
- `E2100` — reserved (previously: `using` may not import methods that require mutable `Self` borrows in the Supported forms).
...
- `E2122` — borrowed control-flow expression is ambiguous in the Supported forms.
