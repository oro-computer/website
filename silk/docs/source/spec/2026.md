...
```silk
fn get_at (xs: arrays::Slice(u8), index: i64) -> u8 | OutOfBounds {
...
fn main () -> int {
  ...
  let xs: arrays::Slice(u8) = { ptr: xs_arr as u64, len: 3 };
 ...
 **final**,
 ...
 - strings starting with `std/` are treated as std-root module specifiers and
 resolve through the configured stdlib root (a trailing `.slk` is accepted for
 compatibility),
 - other strings are treated as dependency module specifiers: the first path
 segment is a package/dependency name, and the remaining path selects a source
 module in that package (see `Packages, Imports, and Exports`).
...
- Arrays / Slices: `T[]`, `T[N]`
 ...
 `Structs, Impl Blocks, and Memory Layout` for the current scalar-slot memory
...
- Module-specifier imports (`import { Name } from "...";`, `import ns from "...";`)
 ...
 - std-root module imports (`from "std/strings"`; a trailing `.slk` is
...
manifest `package.name`. See `Package Manifests (`silk.toml`)`.
...
- Direct `::` imports remain supported for package/ABI-oriented code and are
 documented below.
- Currently:
 - importing a package namespace makes its exported `let` bindings with explicit
 type annotations visible as ordinary, unqualified names in the
 importing module (for example, `import util from "util";` followed by `answer`
 ...
 `import util from "util";`); both unqualified (`answer`) and qualified
 ...
 - explicit imports (namespace imports and module-specifier named imports).
...
#### Module-specifier namespace imports

Use a default import to bind a module or package namespace:
...
Use a selected import when you want a small local surface:

```silk
import { trim as trim_string } from "std/strings";
```
...
These are direct package/ABI import forms. They name the package/export path
that the resolver, ABI surface, and linker see directly.
...
For ordinary application and reusable library code, prefer:
...
This matters most when you use direct package imports such as `import ns_pkg;`,
or when you expect a dependency module import such as `from "logger"` to load a
package root that is not otherwise present.
...
- **Std-root module specifier**: the string begins with `std/`. These imports
 resolve against the configured stdlib root using the slash path.
- **Dependency module specifier**: any other string. The first path segment is a
 package/dependency name, and any remaining path selects a source module below
 that package root. For example, `"logger"` loads the package's default module
 and `"logger/sinks"` loads its `sinks` module.
...
- Ambient imports use the same specifier interpretation rules as other
 ...
 - `std/<path>` is resolved from the configured stdlib root,
...
- File specifiers should include the `.slk` extension explicitly. `std/...`
 strings are std-root module specifiers rather than relative file paths.
...
- Lowering/codegen implements `task` execution using OS threads on `linux/x86_64`
 ...
 `std::runtime::event_loop` API; see `Async Runtime (Hosted)`.
...
- Other punctuation: `?`, `??`, `->`, `=>`, `,`, `;`, `(`, `)`, `{`, `}`, `[`, `]`, `_`, `:`.
 ...
 payload subset; see `Optional`), and
...
- Expression statements: limited to calls, assignments, and increment/decrement
 in the Supported forms (`Expression Statements`).
- Flow control:
