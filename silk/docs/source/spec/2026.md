...
- the repository specification is docs-first,
...
- **Stage 3 (Complete spec + validation)**
  ...
  - End-to-end tests exist (end-to-end test programs + unit test harness as
    appropriate).
...
- add end-to-end test programs (pass + fail cases),
- and ensure diagnostics are stable and documented.

## Language Cheat Sheet

This document summarizes the key syntax and concepts from the Silk language in a condensed form. It is meant as a quick reference; detailed semantics live in the other chapters of this specification.
...
For the authoritative “what works today”, prefer the “Implementation Status”
sections inside the relevant concept documents.
...
- the concept documents in this specification (semantics and checker rules),
- and `Compiler Diagnostics` (error codes for unsupported forms).

### Implementation Status (Read This First)

This specification is the canonical reference for Silk, and many documents describe both:
...
- and `Compiler Diagnostics` for error codes and unsupported forms.
...
See the relevant concept documents in this specification for precise rules.
...
Example (current compiler): the `Packet` above is lowered as 3 scalar
...
A package-level resolver now exists in the compiler and is used by the
...
- The compiler’s type system models optional types, and the parser
  now accepts both:
...
Regions and a richer move/borrow model remain design-in-progress. See `Regions`
and `Borrow Checking (Static Alias and Lifetime Safety)` for current scope.
...
- and “Implementation Status” sections for high-level implementation snapshots.
...
- Configuration:
  ...
  backend (the compiler), with a smaller constant-only fallback for
...
- Building artifacts:
  ...
  - `silk_compiler_build` always performs full front‑end validation for all modules
    ...
    - it then type‑checks the *set* of modules as a unit, taking into account
      ...
      language grammar and semantics documented in this specification,
