...
This spec is written for downstream users: it aims to be readable end-to-end,
while still being precise enough to implement.

For a high-level snapshot of what the reference compiler supports today, see:
[Implementation status](../../docs/?p=compiler/implementation-status).

## Language Cheat Sheet

This document summarizes the key syntax and concepts from Silk in a condensed form. It is meant as a quick reference;
detailed semantics live in the chapters below.
...
For the authoritative view of what the compiler supports today, prefer the
[Implementation status](../../docs/?p=compiler/implementation-status) page and
the “Implementation Status” sections inside the relevant chapters.
...
- References: `&T`.
- Arrays / slices: `T[]`, `T[N]`.
- Range: `range` (range literal expressions: `start..end`, `start..=end`, `start..`, `..end`, `..`).
...
See the relevant chapters below for details.
...
See [External Declarations (`ext`)](../../docs/?p=language/ext) and
[C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk) for full details.
...
- [Formal Grammar Spec](../../docs/?p=language/grammar) (the exact grammar the parser accepts),
- the reference chapters in this specification (semantics and checker rules),
- [Implementation status](../../docs/?p=compiler/implementation-status) (what works end-to-end today),
- and [Compiler diagnostics](../../docs/?p=compiler/diagnostics) (error codes for unsupported forms).

### Implementation Status (Read This First)

This specification is the canonical reference, and many chapters describe both:
...
- [Formal Grammar Spec](../../docs/?p=language/grammar) for syntax,
- [Implementation status](../../docs/?p=compiler/implementation-status) for current end-to-end support,
- and [`examples/`](https://github.com/oro-computer/silk/tree/master/examples) for working programs.
...
- The entrypoint for an executable build is `main` returning `int` (see
  the [`silk` CLI](../../docs/?p=compiler/cli-silk) for rules and supported targets).
...
See [Typed errors](../../docs/?p=language/typed-errors).

#### 4.9 External declarations: `ext`

External function binding (symbol name optional; see [External Declarations (`ext`)](../../docs/?p=language/ext)):
...
`Varargs (Variable Arguments)` and [External Declarations (`ext`)](../../docs/?p=language/ext).

See also: [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk) (C ABI) and `include/silk.h`.
...
`panic` is used for typed errors (see [Typed errors](../../docs/?p=language/typed-errors)):
...
See [Typed errors](../../docs/?p=language/typed-errors) for the Terminal Arm Rule and the supported
pattern forms.
...
See the relevant chapters of this specification for precise rules.
...
- Optionals: `Optional`
- Typed errors: [Typed errors](../../docs/?p=language/typed-errors)
...
- Statements (implemented subset):
  ...
  - `match` as a statement (arms are blocks), used for typed errors as
    specified in [Typed errors](../../docs/?p=language/typed-errors).
...
- Typed errors (`error`, `panic`, and `T | ErrorType...`) are specified in
  [Typed errors](../../docs/?p=language/typed-errors). The current compiler models typed error
...
- A stable, fully-specified string ABI story across the C boundary beyond what
  is documented in [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk).
...
- [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk) (C ABI string representation)
...
- `if` / `else` as statement forms.
- `loop`, `while`, and `for` loops (including the supported `for` forms).
- `break` / `continue` inside loops.
- `return` statements, including “all paths must return” checking for non-`void` functions.
- `match` as an expression for optionals and enums.
- `match` as a statement for typed errors (see [Typed errors](../../docs/?p=language/typed-errors)).
- Expression statements for calls and assignments only
  (see [Expression statements](../../docs/?p=language/flow-expression-statements)).
...
- [Implementation status](../../docs/?p=compiler/implementation-status) (implementation snapshot)
- [Compiler diagnostics](../../docs/?p=compiler/diagnostics) (error codes)
- [`examples/`](https://github.com/oro-computer/silk/tree/master/examples) (working programs)
...
- `panic` exits the function via the typed error system ([Typed errors](../../docs/?p=language/typed-errors)).
...
- The statement form required for typed errors is implemented as part of the
  typed errors feature work and is described in [Typed errors](../../docs/?p=language/typed-errors).
...
typed errors ([Typed errors](../../docs/?p=language/typed-errors)) over enums.
...
- A stable ABI story for passing/returning enums across the C99 boundary (do
  not assume an enum layout until it is specified in [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk)).
...
- [Typed errors](../../docs/?p=language/typed-errors) (`error`, `panic`, and `T | ErrorType...`) (typed errors, not enums)
...
- A stable, fully specified “package build” system outside the current CLI/module-set
  model (see [Package manifests (`silk.toml`)](../../docs/?p=compiler/package-manifests) for current manifest support).

Working programs: see [`examples/`](https://github.com/oro-computer/silk/tree/master/examples).
...
manifest’s `package.name`. See [Package manifests (`silk.toml`)](../../docs/?p=compiler/package-manifests).
...
- **Std-root file specifier**: the string begins with `std/`. These imports
  ...
  [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk)).
...
The compiler currently tracks package boundaries and export surfaces:

- `package` and `import` headers are parsed and validated.
- The export surface is recorded for:
  - values (`export fn`, `export let`, `ext`),
  - Formal Silk theories (`theory`),
  - type aliases (`type`),
...
- Within a module set, calls are type-checked against:
  - all top-level functions in the current package (across all modules of that package), and
  - `export fn` declarations from imported packages.
- Calls to non-exported functions across package boundaries are rejected.

The compiler also builds a package graph for the current module set:

- groups modules into packages (including a default package for modules that omit `package`),
- ensures every package import refers to a package present in the module set, and
- rejects cyclic package graphs (e.g. `package a` importing `b` while `b` imports `a`).

In addition, the compiler constructs per-package export tables for consistency:

- collects exported value declarations into a symbol list,
- rejects duplicate exported names within the same package, except for the
  prototype/definition pairing described above (`export fn name(...) -> T;` +
  `export fn name(...) -> T { ... }`) when the signatures match,
- these export tables are used for validation today, and will be used to drive
  cross-package name resolution.

Future work will tighten and extend this model:

- resolve imports to concrete modules and exported symbols (not just packages),
- ensure cross-package visibility matches the export surface for values and types, and
- propagate package/export metadata into the IR and back-end so linkage and visibility match these rules.
...
within the supported backend subset, see [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk) and
[External Declarations (`ext`)](../../docs/?p=language/ext).
...
- Typed errors are implemented end-to-end for the current front-end and the
  `linux/x86_64` backends (see [Typed errors](../../docs/?p=language/typed-errors)).
...
- Error values exposed through `libsilk.a` should use well-defined C types (e.g. enums or structs) documented in [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk).
...
[Regions](../../docs/?p=language/regions),
[Borrow checking](../../docs/?p=language/borrow-checker), and
[Implementation status](../../docs/?p=compiler/implementation-status)
for current scope.
...
See [External Declarations (`ext`)](../../docs/?p=language/ext) for the external-declaration rules.
...
- [Implementation status](../../docs/?p=compiler/implementation-status) for a high-level implementation snapshot.
...
- Typed error propagation: `?` (postfix).
  ...
  - This propagates typed errors from an error-producing call to the enclosing
    function; see [Typed errors](../../docs/?p=language/typed-errors).
- Ranges: `...`, `..=`, `..`.
- Other punctuation: `?`, `??`, `->`, `=>`, `,`, `;`, `(`, `)`, `{`, `}`, `[`, `]`, `_`, `:`.
  - Currently, `??` is supported for optionals in the
    ...
    error-producing calls (`call()?`; see [Typed errors](../../docs/?p=language/typed-errors)).
...
Our embedding ABI for `libsilk.a` will treat `SilkString` as the canonical C representation; details are further specified in [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk).
...
- `i128` → `SilkI128` (see [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk); `{ lo, hi }` lanes)
- `u128` → `SilkU128` (see [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk); `{ lo, hi }` lanes)
- `int` → `int64_t` (current `linux/x86_64` baseline; do not assume C `int`)
- `f32` → `float`
- `f64` → `double`
- `f128` → `SilkF128` (see [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk); IEEE binary128 bits in `{ lo, hi }`)
- `bool` → `bool` (or `_Bool`)
...
- Keep this document and [C99 ABI and `libsilk.a`](../../docs/?p=compiler/abi-libsilk) in sync with the actual codegen strategy.
...
- Flow control:
  ...
  - `assert`,
  - `panic` (typed errors),
  - `match` statement (typed errors; see [Typed errors](../../docs/?p=language/typed-errors)).
...
`match` (targets not covered by the current enum set map to `Unknown`; see `the standard library`).
...
- [Typed errors](../../docs/?p=language/typed-errors) (unparenthesized `|` in function *signatures*
...
- Mirror the external-declaration semantics described in [External Declarations (`ext`)](../../docs/?p=language/ext).
...
Any deviation from the mappings documented in [External Declarations (`ext`)](../../docs/?p=language/ext) must be justified here and reflected in tests.
