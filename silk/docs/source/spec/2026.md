...
- Regions (fixed-size allocation context):
  - declare: `const region scratch_region: u8[1024];`
  - use: `with scratch_region { let p: &Frame = new Frame{ ... }; }`
...
```silk
// Current subset: `main` may use `?` when it declares a compatible error set.
...
See ``match` Expression` for ordinary value matching and
`Typed Errors (`error`, `panic`, and `T | ErrorType...`)` for the Terminal Arm
Rule when the scrutinee has a typed-error contract.
...
- Statements (implemented subset):
  ...
  - `match` as a statement (arms are blocks), used for both ordinary value
    matching and typed errors as specified in ``match` Expression` and
...
- `match` as an expression for optionals, primitive integers, enums, type
  unions, and recoverable `Result`-style values (``match` Expression`)
...
- The statement form is implemented for:
  ...
  - typed errors in the current subset (`Typed Errors (`error`, `panic`, and `T | ErrorType...`)`).
...
- a multi-module helper seeds each
...
- Recoverable result matching is part of the ordinary-value statement subset
  documented in ``match` Expression`.
...
- Flow control:
  ...
  - `match` statement (ordinary values and typed errors; see
    ``match` Expression` and `Typed Errors (`error`, `panic`, and `T | ErrorType...`)`).
