...
- File imports (named + default): `the runnable fixtures`,
 `the runnable fixtures`,
 `the runnable fixtures`
- Package-path imports (`from pkg::name`): `the runnable fixtures`,
 `the runnable fixtures`
...
import logger from "logger/lib";     // dependency-rooted module path
...
- If the specifier is a quoted **dependency module specifier**, it MUST match a
 ...
 directory; when
 there is no remaining path, `lib.slk` is used.
