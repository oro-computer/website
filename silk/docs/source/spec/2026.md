...
- Structured block:
  ...
  async fn get_dashboard_data() -> Dashboard {
    // Structured blocks are lexical scopes with deterministic live-handle
    // cleanup in the current subset.
    let mut user: User;
...
- the current compiler (what parses, type-checks, and code-generates today).

This tour follows the same approach:

- examples labeled “Example” are intended to compile in the current subset,
...
Current subset (current subset requires initializers; see `E2015`):
...
Current subset (non-capturing expression body):
...
Current subset (block body with explicit return type):
...
Structured blocks (lexical scopes with runtime-backed live-handle cleanup in
the current subset; see `Concurrency`):
...
- Packages and imports:
  ...
  the currently documented surface:
...
- Statements:
  ...
  In the current compiler, the `match` statement form is restricted to a
...
`if` is a **statement** that selects which
...
- In the statement form, a bare identifier arm `Name => { ... }` is reserved
  for binder-style matching, so enum variant arms must use the qualified
  spelling `E::Variant => { ... }` / `E::Variant(x) => { ... }`.
- No guards (`if ...`) are implemented yet.
...
- **`match` expression over enums**:
  ...
  - no guards (`if ...`) yet,
  - and expression-form matches must be *exhaustive* in the current subset:
...
- The same token is also used for:
  ...
  - and ordinary named enums with exactly two declared variants, where the
    first declared variant is the “success” arm:
    - if the first variant is unit, the expression yields that enum value,
    - if the first variant carries exactly one payload, it yields that payload,
    - and if the enum value is the second declared variant, the fallback is
      evaluated.
...
- These forms keep the same validity rules as their statement counterparts:
  - `return` must be valid in the enclosing function and type-check against
    its result type,
  - `break` and `continue` are only valid inside loops.
- This is still a narrow grammar rule for coalescing. It does not make
  `return`, `break`, or `continue` general expressions elsewhere.
...
- Borrowed reference parameters of the form `&Struct` for supported struct values.
...
Silk currently implements a minimal heap model for `new` on `linux/x86_64` and
a small lexical
...
- `yield` on a temporary task handle is eager in the current subset:
  - `yield <task_expr>` where `<task_expr>` is not a named handle drains/joins
    the task and yields its final value `T` (joining dedicated-thread tasks
    only, so the temporary handle does not leak).
...
- `async fn` result types must not contain ordinary borrowed views (`&T` or
  `T[]`), including when nested inside structs, enums, optionals, or function
  types,
  - references to opaque structs (`struct Name;`) are permitted in async
...
- `async { ... }` / `task { ... }` are accepted as structured concurrency surface
  ...
  - these blocks do not create nested executors or inject implicit
    cancellation tokens in the current subset.
...
- If a handle is **not consumed** (`await`/`yield *`), the compiler inserts
  ...
  - `Task(T)` cleanup joins the worker thread for `attr(task=thread)` tasks and
    then frees the handle storage. Pooled/default tasks skip the join.
...
- `yield * <task_handle>` drains all remaining task values and then joins the
  worker thread for cleanup when the task uses `attr(task=thread)`, yielding a
  collected `T[]` in value position. Pooled/default tasks skip the join.
...
introduce surface syntax for structured regions that are intended to become
runtime-backed scheduler scopes in the fuller design.

These forms parse and type-check, but they do
not yet introduce any runtime scheduling; they currently behave like a normal
lexical block.

### Current Runtime Boundaries

This language/spec section describes the shipped concurrency subset and its
current boundaries. Open runtime/compiler architecture backlog is tracked in
`Async Runtime (Hosted)` rather than as language-spec future work.
...
**monomorphized** into concrete, fully specified
...
- Lowering/codegen implements `task` execution using OS threads on `linux/x86_64`
  ...
  - On hosted `linux/x86_64`, `await` is a true suspension point backed by a
    single-threaded executor (fibers).
  - `yield` / `yield *` waits on task output use the same hosted fd-wait path,
    so executor-driven async code suspends the current coroutine instead of
    blocking the executor owner thread.
  - `async { ... }` / `task { ... }` remain lexical scopes, but scope exit now
    performs deterministic live-handle cleanup.
...
- **Block**: `{ stmt* }`, a scope boundary and the unit of structured control
  ...
  be explicit.)
- **Compiler**: the set of features that parse, type-check, and
...
- Other punctuation: `?`, `??`, `->`, `=>`, `,`, `;`, `(`, `)`, `{`, `}`, `[`, `]`, `_`, `:`.
  - Currently, `??` is supported for:
    ...
    - `return`, `break`, and `continue` keep their normal statement validity
      rules and do not become general expressions elsewhere.
    The `?` token is used both in type
...
Status: the current compiler implements this feature under the `ext`
...
- Foreign C code must treat `regexp` values as opaque `(ptr, len)` views rather
  than a stable public bytecode format; the runtime rejects malformed buffers
  as invalid input when they are executed through `std::regex`.
- Only `std::regex::RegExp.compile(...)` produces a runtime-owned regex
  allocation; literal/borrowed/foreign `regexp` views stay borrowed and are
  ignored by the regex free/drop path instead of being freed as if they were
  runtime-owned.
...
build metadata theory `std::runtime::build::build_version_at_least(...)` in
Formal Silk.
...
The `for` loop supports integer range iteration (`start..end` and
...
`loop { ... }`, plus `async loop { ... }` and `task loop { ... }`, are part of
the shipped loop surface.
