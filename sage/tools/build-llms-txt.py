...
def doc_url(item: DocItem) -> str:
    return f"/sage/docs/?p={quote(item.id)}"
...
def load_items(docs_root: Path) -> list[DocItem]:
    ...
    if not index_path.exists():
        raise SystemExit(
            f"Missing {index_path}. Run: python3 website/sage/tools/build-indexes.py"
        )
...
def main():
    ...
    docs_root = repo_root / "website" / "sage" / "docs"
    out_path = repo_root / "website" / "sage" / "llms.txt"

    items = load_items(docs_root)
    generated_at = datetime.now(timezone.utc).isoformat(timespec="seconds")

    lines: list[str] = []
    lines.append("Sage · LLMS Pack")
    lines.append("================")
    lines.append("")
    lines.append("This file concatenates the full Sage documentation hosted on this website,")
    lines.append("so an LLM can answer questions using the same source of truth as readers.")
    lines.append("")
    lines.append(f"Generated: {generated_at}")
    lines.append("")
    lines.append("How to link:")
    lines.append("- Docs: /sage/docs/?p=<id>")
    lines.append("")
    lines.append("Table of contents")
    lines.append("-----------------")
    lines.append("")
    lines.append("Sage Docs")
